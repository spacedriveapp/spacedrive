...
use sd_p2p::{Identity, NetworkManager, NetworkManagerEvent, PeerMetadata};
use tokio::sync::mpsc;
...
#[tokio::main]
async fn main() {
    // get_metadata is a closure which returns metadata for the current node. This is a closure so the data can be modified at runtime by the application.
    let get_metadata = || PeerMetadata {
    ...
    let nm = NetworkManager::new(APPLICATION_NAME, identity, tx, get_metadata)
        .await
        .unwrap();
    println!(
