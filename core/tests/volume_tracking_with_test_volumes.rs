...
async fn test_real_volume_tracking_lifecycle() {
	...
	info!(
		"Found test volume: {} ({})",
		our_volume.name, our_volume.fingerprint
	);
...
async fn test_volume_capacity_scenarios() {
	...
	for (name, size_mb) in test_cases {
		...
		let test_volume = match TestVolumeBuilder::new(name).size_mb(size_mb).build().await {
			Ok(vol) => vol,
			Err(e) => {
		...
		.find(|v| v.mount_point == ram_volume.mount_point)
	{
			...
			.await
		{
			Ok(sd_core::ops::volumes::speed_test::output::VolumeSpeedTestOutput {
				...
				info!(
					"RAM Disk speeds - Read: {:?} MB/s, Write: {:?} MB/s",
...
async fn test_volume_mount_unmount_tracking() {
	...
	let test_volume = manager
		.create_volume(config.clone())
		.await
		.expect("Failed to create test volume");
	...
	core.volumes
		.track_volume(
			&library,
			&volume.fingerprint,
			Some("Remountable Volume".to_string()),
		)
		.await
		.expect("Failed to track volume");
...
async fn test_concurrent_volume_operations() {
	...
	for (i, result) in results.iter().enumerate() {
		...
		assert!(
			...
			"Library {} failed to track volume",
			i
		);
	}
	...
	for (i, library) in libraries.iter().enumerate() {
		...
		let our_volume = tracked
			...
			.find(|v| v.fingerprint == volume.fingerprint)
			.expect(&format!(
				"Library {} should have our test volume tracked",
				i
			));
		...
		assert_eq!(
			...
			"Library {} should have correct volume name",
			i
		);
	}
