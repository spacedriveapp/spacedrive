...
impl SyncTestSetup {
	...
	async fn new() -> anyhow::Result<Self> {
		...
		let core_a = Core::new(temp_dir_a.path().to_path_buf()).await.map_err(|e| anyhow::anyhow!("{}", e))?;
		let device_a_id = core_a.device.device_id()?;

		let core_b = Core::new(temp_dir_b.path().to_path_buf()).await.map_err(|e| anyhow::anyhow!("{}", e))?;
		let device_b_id = core_b.device.device_id()?;

		let library_a = core_a.libraries.create_library_no_sync("Test Library A", None, core_a.context.clone()).await?;
		let library_b = core_b.libraries.create_library_no_sync("Test Library B", None, core_b.context.clone()).await?;
		...
		library_a.init_sync_service(device_a_id, transport_a.clone() as Arc<dyn NetworkTransport>).await?;
		library_b.init_sync_service(device_b_id, transport_b.clone() as Arc<dyn NetworkTransport>).await?;
	...
	async fn assert_tag_count(
		...
		let count = entities::tag::Entity::find()
			...
			.await?
			.len();
		assert_eq!(count, expected, "expected {} tags, found {}", expected, count);
		Ok(())
	}

	/// Get SharedChange messages from A to B
	async fn get_shared_change_messages(
		&self,
	) -> Vec<sd_core::infra::sync::SharedChangeEntry> {
		self.transport_a
...
async fn test_sync_location_device_owned_state_based() -> anyhow::Result<()> {
	...
	let entry = setup.create_entry_with_uuid(entry_uuid, "Test Location", 1, &setup.library_a, None).await?;

	// Manually create entry on device B to satisfy FK dependency
	setup.create_entry_with_uuid(entry_uuid, "Test Location", 1, &setup.library_b, None).await?;
	...
	setup.library_a.sync_model_with_db(&location_record, ChangeType::Insert, setup.library_a.db().conn()).await?;
	...
	assert!(location_on_b.is_some(), "location failed to sync to device B");
...
async fn test_sync_tag_shared_hlc_based() -> anyhow::Result<()> {
	...
	let tag = setup.create_tag("Vacation", setup.device_a_id, &setup.library_a).await?;
...
async fn test_sync_entry_with_location() -> anyhow::Result<()> {
	...
	let location_entry = setup.create_entry("Test Location", 1, &setup.library_a, None).await?;
	...
	let entry = setup.create_entry("test_file.txt", 0, &setup.library_a, Some(location_entry.id)).await?;
	setup.library_a.sync_model_with_db(&entry, ChangeType::Insert, setup.library_a.db().conn()).await?;
	setup.wait_for_sync(Duration::from_secs(2)).await?;

	let messages = setup.transport_a.get_messages_between(setup.device_a_id, setup.device_b_id).await;
	...
	assert!(state_changes.iter().any(|m| m == "entry"), "no entry state change");
...
async fn test_sync_backfill_includes_pre_sync_data() -> anyhow::Result<()> {
	...
	for (i, &uuid) in pre_sync_tags.iter().enumerate() {
		setup.create_tag_with_uuid(uuid, format!("PreSync {}", i), setup.device_a_id, &setup.library_a).await?;
	}

	let post_sync_tags: Vec<Uuid> = (0..2).map(|_| Uuid::new_v4()).collect();
	for (i, &uuid) in post_sync_tags.iter().enumerate() {
		let tag = setup.create_tag_with_uuid(uuid, format!("PostSync {}", i), setup.device_a_id, &setup.library_a).await?;
		setup.library_a.sync_model(&tag, ChangeType::Insert).await?;
	}

	setup.assert_tag_count(5, &setup.library_a).await?;

	let log_entries = setup.library_a.sync_service().unwrap().peer_sync().peer_log().get_since(None).await?;
	assert_eq!(log_entries.len(), 2, "peer log should have 2 entries");

	let full_state = setup.library_a.sync_service().unwrap().peer_sync().get_full_shared_state().await?;
...
async fn test_sync_transitive_three_devices() -> anyhow::Result<()> {
	let _ = tracing_subscriber::fmt().with_env_filter("sd_core=debug").with_test_writer().try_init();
	...
	let core_a = Core::new(temp_dir_a.path().to_path_buf()).await.map_err(|e| anyhow::anyhow!("{}", e))?;
	let core_b = Core::new(temp_dir_b.path().to_path_buf()).await.map_err(|e| anyhow::anyhow!("{}", e))?;
	let core_c = Core::new(temp_dir_c.path().to_path_buf()).await.map_err(|e| anyhow::anyhow!("{}", e))?;

	let (device_a_id, device_b_id, device_c_id) = (core_a.device.device_id()?, core_b.device.device_id()?, core_c.device.device_id()?);

	let library_a = core_a.libraries.create_library_no_sync("Shared Library", None, core_a.context.clone()).await?;
	let library_b = core_b.libraries.create_library_no_sync("Shared Library", None, core_b.context.clone()).await?;
	let library_c = core_c.libraries.create_library_no_sync("Shared Library", None, core_c.context.clone()).await?;
	...
	let transport_a = MockTransport::new(device_a_id, vec![device_b_id], queues.clone(), history.clone());
	let transport_b = MockTransport::new(device_b_id, vec![device_a_id, device_c_id], queues.clone(), history.clone());
	let transport_c = MockTransport::new(device_c_id, vec![device_b_id], queues.clone(), history.clone());

	library_a.init_sync_service(device_a_id, transport_a.clone()).await?;
	library_b.init_sync_service(device_b_id, transport_b.clone()).await?;
	library_c.init_sync_service(device_c_id, transport_c.clone()).await?;
	...
	let tag_on_b = entities::tag::Entity::find().filter(entities::tag::Column::Uuid.eq(tag_uuid)).one(library_b.db().conn()).await?;
	...
	let tag_on_c = entities::tag::Entity::find().filter(entities::tag::Column::Uuid.eq(tag_uuid)).one(library_c.db().conn()).await?;
...
async fn test_connection_state_tracking() -> anyhow::Result<()> {
	...
	let device_b_on_a = setup.find_device(setup.device_b_id, &setup.library_a).await?;
	let device_a_on_b = setup.find_device(setup.device_a_id, &setup.library_b).await?;
	assert_eq!(device_b_on_a.is_online, false);
	assert_eq!(device_a_on_b.is_online, false);

	setup.set_device_online(setup.device_b_id, &setup.library_a).await?;
	setup.set_device_online(setup.device_a_id, &setup.library_b).await?;
	...
	setup.set_device_offline(setup.device_b_id, &setup.library_a).await?;
	setup.set_device_offline(setup.device_a_id, &setup.library_b).await?;
...
async fn test_watermark_reconnection_sync() -> anyhow::Result<()> {
	...
	for i in 0..3 {
		let tag = setup.create_tag(format!("Initial {}", i), setup.device_a_id, &setup.library_a).await?;
		setup.library_a.sync_model(&tag, ChangeType::Insert).await?;
	}
	setup.wait_for_sync(Duration::from_secs(2)).await?;
	setup.assert_tag_count(3, &setup.library_b).await?;

	let last_hlc = setup.get_shared_change_messages().await
		.into_iter()
		.map(|e| e.hlc)
		.last()
	...
	for i in 0..2 {
		let tag = setup.create_tag(format!("New {}", i), setup.device_a_id, &setup.library_a).await?;
		setup.library_a.sync_model(&tag, ChangeType::Insert).await?;
		new_tags.push(tag.uuid);
	}

	let messages_before = setup.transport_a.get_messages_between(setup.device_a_id, setup.device_b_id).await.len();
	...
	tokio::spawn({
		...
		async move {
			transport_b.send_sync_message(device_a_id, request).await.unwrap();
		}
	});

	setup.wait_for_sync(Duration::from_secs(2)).await?;

	let all_messages = setup.transport_a.get_messages_between(setup.device_a_id, setup.device_b_id).await;
	let responses: Vec<_> = all_messages
		.iter()
		.skip(messages_before)
		.filter_map(|msg| {
			if let SyncMessage::SharedChangeResponse { entries, current_state, .. } = msg {
	...
	assert!(responses.iter().all(|(_, state)| state.is_none()), "incremental sync included full state");
...
async fn test_automatic_watermark_exchange_on_reconnection() -> anyhow::Result<()> {
	...
	sync_b.peer_sync().exchange_watermarks_and_catchup(setup.device_a_id).await?;
	setup.wait_for_sync(Duration::from_secs(2)).await?;

	use sd_core::service::network::protocol::sync::messages::SyncMessage;
	let messages_b_to_a = setup.transport_b.get_messages_between(setup.device_b_id, setup.device_a_id).await;
	let messages_a_to_b = setup.transport_a.get_messages_between(setup.device_a_id, setup.device_b_id).await;

	assert!(messages_b_to_a.iter().any(|m| matches!(m, SyncMessage::WatermarkExchangeRequest { .. })), "no watermark request");
	assert!(messages_a_to_b.iter().any(|m| matches!(m, SyncMessage::WatermarkExchangeResponse { .. })), "no watermark response");
	assert!(messages_b_to_a.iter().any(|m| matches!(m, SyncMessage::SharedChangeRequest { .. })), "no shared change request");
...
async fn test_concurrent_tag_updates_hlc_conflict_resolution() -> anyhow::Result<()> {
	...
	let tag_uuid = Uuid::new_v4();
	let tag_a = setup.create_tag_with_uuid(tag_uuid, "Version A", setup.device_a_id, &setup.library_a).await?;
	let tag_b = setup.create_tag_with_uuid(tag_uuid, "Version B", setup.device_b_id, &setup.library_b).await?;

	tokio::time::sleep(Duration::from_millis(50)).await;
	setup.library_a.sync_model(&tag_a, ChangeType::Insert).await?;

	tokio::time::sleep(Duration::from_millis(100)).await;
	setup.library_b.sync_model(&tag_b, ChangeType::Insert).await?;
	...
	assert_eq!(tag_on_a.canonical_name, tag_on_b.canonical_name, "tags did not converge");
...
async fn test_sync_update_and_delete_operations() -> anyhow::Result<()> {
	...
	assert_eq!(tag_on_b.description, Some("Original description".to_string()));
	...
	setup.library_a.sync_model(&updated_tag, ChangeType::Update).await?;
	setup.wait_for_sync(Duration::from_secs(2)).await?;

	let tag_on_b = setup.assert_tag_exists(tag_uuid, &setup.library_b).await?;
	assert_eq!(tag_on_b.canonical_name, "Updated");
	assert_eq!(tag_on_b.description, Some("Updated description".to_string()));

	entities::tag::Entity::delete_by_id(updated_tag.id).exec(setup.library_a.db().conn()).await?;
	setup.library_a.sync_model(&updated_tag, ChangeType::Delete).await?;
	setup.wait_for_sync(Duration::from_secs(2)).await?;
