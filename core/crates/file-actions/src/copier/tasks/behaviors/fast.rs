...
use super::{CopyBehavior, FAST_COPY_SIZE_THRESHOLD, utils::is_same_filesystem};
...
impl CopyBehavior for FastCopyBehavior {
    async fn copy_file(
        ...
        let metadata = fs::metadata(&source).await.map_err(|e| JobError::IO(e.into()))?;
        let total_bytes = metadata.len();
        let file_name = source.as_ref().file_name().unwrap_or_default().to_string_lossy().to_string();
    ...
    fn is_suitable(&self, source: impl AsRef<Path>, target: impl AsRef<Path>) -> bool {
        ...
        if let Ok(metadata) = std::fs::metadata(source.as_ref()) {
            metadata.len() <= FAST_COPY_SIZE_THRESHOLD && 
            is_same_filesystem(source, target)
        } else {
            false
        }
    }
