...
use sd_core_job_system::report::ReportInputMetadata;
...
pub async fn create_file_path(
	...
	metadata: sd_core_file_helper::FilePathMetadata,
) -> Result<file_path::Data, sd_core_file_helper::FilePathError> {
	...
	let location = db
		...
		.exec()
		.await?
		.ok_or(sd_core_file_helper::FilePathError::LocationNotFound(
			location_id,
		))?;
