...
use std::{path::Path, cmp::min, io, str::FromStr, path::PathBuf};
use tokio::{fs::File, io::{AsyncReadExt, AsyncSeekExt, SeekFrom}};
...
pub async fn handle_custom_uri(node: &Node, req: Request<Vec<u8>>) -> Result<Response<Vec<u8>>, HandleCustomUriError> {
    ...
    match path.first().copied() {
        Some("thumbnail") => {
            ...
            let mut file = File::open(&filename).await.map_err(|_| HandleCustomUriError::NotFound("file"))?;
        ...
        Some("file") => {
            let library_id = path.get(1).map(|id| Uuid::from_str(&id).ok()).flatten().ok_or_else(|| HandleCustomUriError::BadRequest("Invalid number of parameters. Missing library_id!"))?;
            let location_id = path.get(2).map(|id| id.parse::<i32>().ok()).flatten().ok_or_else(|| HandleCustomUriError::BadRequest("Invalid number of parameters. Missing location_id!"))?;
            let file_path_id = path.get(3).map(|id| id.parse::<i32>().ok()).flatten().ok_or_else(|| HandleCustomUriError::BadRequest("Invalid number of parameters. Missing file_path_id!"))?;

            let library = node.library_manager.get_ctx(library_id).await.ok_or_else(|| HandleCustomUriError::NotFound("library"))?;
            let file_path = library.db
                .file_path()
                .find_first(vec![
                ...
                let location_materialized_path = file_path.location.expect("unreachable location not fetched").local_path.ok_or_else(|| {
                    warn!("Location '{}' doesn't have local path set", file_path.location_id);
                    HandleCustomUriError::BadRequest("Location doesn't have `local_path` set!")
                })?;
            let file_path_materialized_path = PathBuf::from(location_materialized_path).join(&file_path.materialized_path);
            let mut file = File::open(file_path_materialized_path).await.map_err(|_| HandleCustomUriError::NotFound("file"))?;
            ...
            let (mime_type, is_video) = match file_path.extension.as_deref() {
                ...
                _ => {
                    return Err(HandleCustomUriError::BadRequest("TODO: This filetype is not supported because of the missing mime type!"));
                }
            };
            ...
            match is_video {
                true => {
                    ...
                    if let Some(range) = req.headers().get("range") {
                        ...
                        let range = HttpRange::parse(range.to_str().or_else(|_| Err(HandleCustomUriError::BadRequest("Error passing range header!")))?, file_size).or_else(|_| Err(HandleCustomUriError::BadRequest("Error passing range!")))?;
                    ...
                    Ok(response.header("Content-type", mime_type).status(status_code).body(buf)?)
                },
                false => {
...
impl HandleCustomUriError {
    pub fn into_response(self) -> http::Result<Response<Vec<u8>>> {
        match self {
            ...
            HandleCustomUriError::NotFound(resource) => 
                Response::builder()
                    .header("Content-Type", "text/plain")
                    .status(StatusCode::NOT_FOUND)
                    .body(format!("Resource '{}' not found", resource).as_bytes().to_vec()),
        }
    }
}