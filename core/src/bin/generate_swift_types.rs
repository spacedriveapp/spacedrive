...
fn main() -> Result<(), Box<dyn std::error::Error>> {
	...
	let generated_content = generate_swift_api_code(&api_structure, &operations, &queries, &types)?;
...
fn generate_swift_api_code(
	...
	operations: &[sd_core::ops::type_extraction::OperationMetadata],
	queries: &[sd_core::ops::type_extraction::QueryMetadata],
	...
	// Generate all the individual types using Specta Swift
	...
	// Add the Event enum to our type collection and generate it
	let mut event_types = specta::TypeCollection::default();
	event_types.register_mut::<sd_core::infra::event::Event>();
	event_types.register_mut::<sd_core::infra::event::FsRawEventKind>();
	event_types.register_mut::<sd_core::infra::event::FileOperation>();

	let event_types_swift = swift.export(&event_types)?;
	swift_code.push_str(&event_types_swift);
	swift_code.push_str("\n");
