//! Migration to create location service settings tables
//!
//! Creates tables for per-location service configuration (watcher, stale detector, sync)
//! and tracking watcher lifecycle state and stale detection run history.
...
impl MigrationTrait for Migration {
		...
		manager
			.create_table(
				Table::create()
					...
					.foreign_key(
						ForeignKey::create()
							.name("fk_location_service_settings_location_id")
							.from(LocationServiceSettings::Table, LocationServiceSettings::LocationId)
		...
		manager
			.create_table(
				Table::create()
					...
					.foreign_key(
						ForeignKey::create()
							.name("fk_location_watcher_state_location_id")
		...
		manager
			.create_table(
				Table::create()
					...
					.foreign_key(
						ForeignKey::create()
							.name("fk_stale_detection_runs_location_id")
	...
	async fn down(&self, manager: &SchemaManager) -> Result<(), DbErr> {
		...
		manager
			...
			.await?;
		manager
			.drop_table(Table::drop().table(LocationServiceSettings::Table).to_owned())
			.await?;
