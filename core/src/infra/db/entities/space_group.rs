...
impl Syncable for Model {
	...
	// FK Lookup Methods (space_group is FK target for space_items)
	...
	async fn lookup_uuid_by_id(
		...
		Ok(Entity::find_by_id(id).one(db).await?.map(|g| g.uuid))
	}
...
crate::register_syncable_shared!(Model, "space_group", "space_groups");
