...
impl ActionHandler for ValidationHandler {
    async fn validate(
        ...
        if let crate::infra::actions::Action::FileValidate { action, .. } = action {
    ...
    async fn execute(
        ...
        if let crate::infra::actions::Action::FileValidate { library_id, action } = action {
    ...
    fn can_handle(&self, action: &crate::infra::actions::Action) -> bool {
        matches!(action, crate::infra::actions::Action::FileValidate { .. })
    }
