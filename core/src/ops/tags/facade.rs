...
use crate::{
	domain::tag::{
		PrivacyLevel, RelationshipType, Tag, TagApplication, TagError, TagSource, TagType,
	},
	infra::db::Database,
	ops::{metadata::manager::UserMetadataManager, tags::manager::TagManager},
};
...
impl TaggingFacade {
	...
	pub async fn create_namespaced_tag(
		...
		let mut tag = self
			.tag_manager
			.create_tag(name, Some(namespace), device_id)
			.await?;
	...
	pub async fn create_tag_with_variants(
		...
		let mut tag = self
			.tag_manager
			.create_tag(canonical_name, namespace, device_id)
			.await?;
	...
	pub async fn create_tag_hierarchy(
		...
		for (name, namespace) in hierarchy {
			let tag = self
				.tag_manager
				.create_tag(name, namespace, device_id)
				.await?;
	...
	pub async fn tag_entry(
		...
		self.metadata_manager
			.apply_user_semantic_tags(entry_id, &applied_tag_ids, device_id)
			.await?;
	...
	pub async fn apply_ai_tags(
		...
		self.metadata_manager
			.apply_ai_semantic_tags(entry_id, tag_suggestions.clone(), device_id)
			.await?;
	...
	pub async fn suggest_tags_for_entry(
		...
		let existing_applications = self
			.metadata_manager
			.get_semantic_tags_for_entry(entry_id)
			.await?;
		let existing_tag_ids: Vec<Uuid> =
			existing_applications.iter().map(|app| app.tag_id).collect();
