...
impl SidecarManager {
	...
	pub async fn exists(
		...
		let path = self
			.compute_path(library_id, content_uuid, kind, variant, format)
			.await?;
		...
		if let Some(sidecar) = existing {
			match sidecar.status.as_str() {
				...
				"failed" => {
					...
					self.enqueue_generation(library, content_uuid, kind, variant, format)
						.await?;
	...
	pub async fn convert_reference_to_owned(
		...
		for sidecar in reference_sidecars {
			if let Some(source_entry_id) = sidecar.source_entry_id {
				...
				active.rel_path =
					ActiveValue::Set(target_path.relative_path.to_string_lossy().to_string());
		...
		while let Some(h0_entry) = shard_dirs.next_entry().await? {
			...
			while let Some(h1_entry) = h0_dirs.next_entry().await? {
				...
				while let Some(content_entry) = content_dirs.next_entry().await? {
					...
					if let Err(e) = self
						.scan_content_sidecars(library, &content_uuid, &content_entry.path())
						.await
					{
