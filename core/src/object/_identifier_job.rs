...
impl StatefulJob for FileIdentifierJob {
	...
	async fn execute_step(
		...
		for existing_object in &existing_objects {
			let location_id = state.init.location_id;
			let id = *cas_lookup.get(&existing_object.cas_id).unwrap();
				...
				.run(|db| async move {
					sync.write_op(
						...
						sync.owned_update(
							...
							[("object", json!({ "cas_id": existing_object.cas_id }))],
						),
					)
					.await?;
					...
					db.file_path()
						.update(
							file_path::location_id_id(location_id, id),
							vec![file_path::object::connect(object::id::equals(
								existing_object.id,
							))],
						)
						.exec()
		...
		if !new_objects.is_empty() {
			...
			let created_files: Vec<FileCreated> = db
				._transaction()
				.run(|db| async move {
					let mut ops = Vec::with_capacity(4 * new_objects.len());

					for object in &new_objects {
						ops.push(sync.shared_create("Object", json!(object.cas_id)));

						for (key, value) in [
							("size_in_bytes", json!(object.size_in_bytes.to_string())),
							("date_created", json!(object.date_created)),
							("kind", json!(object.kind.int_value() as i64)),
						] {
					...
					let (owned, shared) = sync.write_ops(&db, ops).await?;

					db._batch(owned).await?;
					db._batch(shared).await?;
			...
			for created_file in created_files {
				let id = *cas_lookup.get(&created_file.cas_id).unwrap();
				let location_pub_id = &data.location.pub_id;
					...
					.run(|db| async move {
						sync.write_op(
							...
							sync.owned_update(
								"FilePath",
								json!({
									"location_id": location_pub_id,
									"id": id
								}),
								[("object", json!({ "cas_id": created_file.cas_id }))],
							),
						)
						.await?;
