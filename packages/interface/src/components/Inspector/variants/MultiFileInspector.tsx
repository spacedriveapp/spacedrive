import { Calendar, Files, Folder, Tag as TagIcon } from "@phosphor-icons/react";
...
import { Divider, InfoRow, Section, Tag } from "../Inspector";
...
export function MultiFileInspector({ files }: MultiFileInspectorProps) {
  ...
  const aggregatedData = useMemo(() => {
    ...
    return {
      totalSize,
      kindCounts: Array.from(kindCounts.entries()).sort((a, b) => b[1] - a[1]),
    ...
    <div className="no-scrollbar mask-fade-out flex flex-col space-y-5 overflow-x-hidden overflow-y-scroll pb-10">
          ...
          {thumbnailFiles.map((file, i, thumbs) => (
            <div
              className={clsx(
                ...
                i === 0 && thumbs.length > 1 && "!h-[76%] !w-[76%] z-30",
                i === 1 && "!h-4/5 !w-4/5 z-20 rotate-[-5deg]",
                i === 2 && "!h-[84%] !w-[84%] z-10 rotate-[7deg]"
              )}
              key={file.id}
            >
              <FileComponent.Thumb
                className={clsx(
                  ...
                  thumbs.length > 1 && "shadow-black/20 shadow-md"
                )}
                file={file}
      ...
      <div className="px-2 pb-3">
        <div className="text-center">
          <h2 className="font-semibold text-lg text-sidebar-ink">
      ...
      {/* Summary section */}
      <Section icon={Files} title="Summary">
        <InfoRow
          label="Total Size"
          value={formatBytes(aggregatedData.totalSize)}
        />
      ...
      {aggregatedData.kindCounts.length > 0 && (
        <Section icon={Folder} title="Types">
              ...
              return (
                ...
                >
                  <Tag color={tag.color}>
                    {tag.name}
                    {coverage < 1 && (
                      <span className="ml-1 text-[10px]">({tag.count})</span>
                    )}
                  </Tag>
                </div>
              );
