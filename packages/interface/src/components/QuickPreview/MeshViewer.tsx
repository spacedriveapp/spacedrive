...
const CAMERA_LOOK_AT = [-0.006_97, -0.005_33, -0.618_58] as const;
      ...
      try {
        ...
        if (!cancelled) {
          ...
          if (splatMesh?.calculatedSceneCenter) {
            ...
            viewer.camera.position.set(center.x, center.y, center.z - 0.5);
            viewer.camera.lookAt(center.x, center.y, center.z);
            viewer.camera.updateProjectionMatrix();
            viewer.controls.update();

            console.log("[GaussianSplatViewer] Setup for raycast:", {
              center: {
                x: center.x,
                y: center.y,
                z: center.z,
                ...
                if (outHits.length > 0) {
                  console.log(
                    `[GaussianSplatViewer] ✓ Raycast SUCCESS (attempt ${retryCount})!`,
                    {
                      ...
                      allHits: outHits.map((h: any, i: number) => ({
                        index: i,
                        origin: {
                          x: h.origin?.x,
                          y: h.origin?.y,
                      ...
                      calculatedCenter: {
                        x: viewer.splatMesh.calculatedSceneCenter.x,
                        y: viewer.splatMesh.calculatedSceneCenter.y,
                        z: viewer.splatMesh.calculatedSceneCenter.z,
                      },
                    }
                  );
                  ...
                  console.log("[GaussianSplatViewer] Using closest hit:", {
                    origin: {
                  ...
                  if (!initialRaycastFocalPointRef.current) {
                    ...
                    onResetReady?.(() => {
                      if (
                        ...
                        const initial = initialRaycastFocalPointRef.current;
                        const current = viewer.controls.target;

                        // Check if we're already at the initial point (avoid unnecessary transition)
                        const distance = Math.sqrt(
                          (current.x - initial.x) ** 2 +
                            (current.y - initial.y) ** 2 +
                            (current.z - initial.z) ** 2
                        );
                        ...
                        viewer.previousCameraTarget.copy(current);
                        viewer.nextCameraTarget.copy(initial);
                  ...
                  const actualDistance = Math.sqrt(
                    (currentCameraPos.x - intersectionPoint.x) ** 2 +
                      (currentCameraPos.y - intersectionPoint.y) ** 2 +
                      (currentCameraPos.z - intersectionPoint.z) ** 2
                  );

                  // Set both distance refs to the actual current distance
                  cameraDistanceRef.current = actualDistance;
                  currentCameraDistanceRef.current = actualDistance;
                  ...
                  viewer.controls.target.copy(intersectionPoint);
                  viewer.controls.update();
  ...
  useEffect(() => {
    ...
    const startAnimation = () => {
      if (
        ...
        startTimeoutId = setTimeout(startAnimation, 100) as unknown as number;
        return;
      }
      ...
      console.log("[Animation Start] Initial focal point:", initialFocalPoint);
      ...
      const animate = () => {
        ...
        const angle =
          Math.sin(elapsed * swaySpeedRef.current) * swayAmountRef.current;
        ...
        if (transition.active) {
          ...
          camera.position.x = focalPoint.x + transition.cameraOffset.x;
          camera.position.y = focalPoint.y + transition.cameraOffset.y;
          camera.position.z = focalPoint.z + transition.cameraOffset.z;
          ...
          if (progress >= 1) {
            ...
            if (controls) {
              controls.target.set(focalPoint.x, focalPoint.y, focalPoint.z);
            }
            console.log(
          ...
          camera.position.x =
            focalPoint.x + Math.sin(angle) * currentCameraDistanceRef.current;
          camera.position.z =
            focalPoint.z + -Math.cos(angle) * currentCameraDistanceRef.current;
      ...
      requestAnimationFrame(() => {
        ...
        camera.position.set(fp.x, fp.y, fp.z - cameraDistanceRef.current);
...
export function MeshViewerUI({
  ...
  if (!isGaussianSplat) {
    return (
      <>
        <div className="pointer-events-none absolute top-4 left-4 rounded-lg bg-black/80 px-3 py-1.5 font-medium text-sm text-white backdrop-blur-xl">
          3D Mesh
        </div>
        <div className="pointer-events-none absolute bottom-4 left-4 rounded-lg bg-black/80 px-3 py-2 text-white/70 text-xs backdrop-blur-xl">
  ...
  return (
    <>
      {/* Button controls */}
      <div className="pointer-events-auto absolute right-4 bottom-4 flex gap-1">
        <TopBarButton
          active={showSettings}
          activeAccent={true}
          icon={Sliders}
      ...
      {showSettings && (
        <div className="pointer-events-auto absolute right-4 bottom-16 flex min-w-[240px] flex-col gap-2 rounded-lg border border-app-line bg-app-box/95 p-3 shadow-lg backdrop-blur-lg">
          {/* Sway amount slider */}
          <div className="flex flex-col gap-1">
            <div className="flex items-center justify-between">
              <label className="text-ink-dull text-xs">Sway Amount</label>
              <span className="font-mono text-ink-dull text-xs">
                {swayAmount.toFixed(2)}
              </span>
            </div>
            <input
              className="h-1.5 w-full cursor-pointer appearance-none rounded-full bg-app-button [&::-webkit-slider-thumb]:h-3 [&::-webkit-slider-thumb]:w-3 [&::-webkit-slider-thumb]:cursor-pointer [&::-webkit-slider-thumb]:appearance-none [&::-webkit-slider-thumb]:rounded-full [&::-webkit-slider-thumb]:bg-accent"
              max="0.5"
              min="0"
              onChange={(e) => setSwayAmount(Number.parseFloat(e.target.value))}
              step="0.01"
              type="range"
              value={swayAmount}
            />
          ...
          <div className="flex flex-col gap-1">
            <div className="flex items-center justify-between">
              <label className="text-ink-dull text-xs">Speed</label>
              <span className="font-mono text-ink-dull text-xs">
                {swaySpeed.toFixed(2)}
              </span>
            </div>
            <input
              ...
              max="2"
              min="0.1"
              onChange={(e) => setSwaySpeed(Number.parseFloat(e.target.value))}
              step="0.1"
              type="range"
              value={swaySpeed}
            />
          ...
          <div className="flex flex-col gap-1">
            <div className="flex items-center justify-between">
              <label className="text-ink-dull text-xs">Distance</label>
              <span className="font-mono text-ink-dull text-xs">
            ...
            <input
              ...
              min="0.2"
              onChange={(e) =>
                setCameraDistance(Number.parseFloat(e.target.value))
              }
              step="0.05"
              type="range"
    ...
    fetch(url, { signal: abortController.signal })
      ...
      .catch((error) => {
        ...
        if (error.name !== "AbortError") {
          console.error("[MeshViewer] Error detecting format:", error);
        }
        setLoading(false);
      });
    ...
    return (
      <div className="flex h-full w-full items-center justify-center">
        <div className="text-center">
          ...
          {loading && (
            <div className="mt-4 text-ink-dull text-sm">
              Loading 3D model...
            </div>
          )}
        </div>
