...
function ImageRenderer({ file, onZoomChange }: ContentRendererProps) {
  ...
  return (
    ...
    >
      ...
      {zoom > 1 && (
        <div className="absolute top-4 left-4 z-10 rounded-lg bg-app-box/80 px-3 py-1.5 font-medium text-ink text-sm backdrop-blur-xl">
          {Math.round(zoom * 100)}%
        </div>
      )}
    ...
    return (
      <div className="flex h-full w-full items-center justify-center">
        <div className="text-center">
          ...
          <div className="mt-6 font-medium text-ink text-lg">{file.name}</div>
        </div>
      </div>
    );
  }

  return <AudioPlayer file={file} src={audioUrl} />;
}
  ...
  return (
    <div className="flex h-full w-full items-center justify-center">
      <div className="text-center">
        <FileComponent.Thumb file={file} size={200} />
        <div className="mt-6 font-medium text-ink text-lg">{file.name}</div>
        <div className="mt-2 text-ink-dull text-sm capitalize">
          {getContentKind(file) ?? "unknown"}
        </div>
        <div className="mt-1 text-ink-dull text-xs">
...
function TextRenderer({ file }: ContentRendererProps) {
    ...
    return (
      <div className="flex h-full w-full items-center justify-center">
        <div className="text-center">
          ...
          <div className="mt-4 font-medium text-ink text-lg">{file.name}</div>
          <div className="mt-2 text-ink-dull text-sm">Loading...</div>
        </div>
      </div>
    );
  }
  ...
  return (
    <>
      <WithPrismTheme />
      <TextViewer
        className="h-full w-full overflow-auto bg-app p-4 text-ink"
