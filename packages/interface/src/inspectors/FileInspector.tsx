  ...
  return (
    <div className="no-scrollbar mask-fade-out flex flex-col space-y-5 overflow-x-hidden overflow-y-scroll pb-10">
      ...
      <Section title="Details" icon={Info}>
        ...
        {file.extension && (
          <InfoRow label="Extension" value={file.kind?.File?.extension} />
        )}
        <InfoRow label="Kind" value={String(file?.content_identity?.kind)} />
      </Section>
      ...
      <Section title="Storage" icon={HardDrive}>
        <InfoRow
          label="Path"
          value={
            file.sd_path.type === "Physical"
              ? String(file.sd_path.path)
              : file.sd_path.type === "Cloud"
                ? String(file.sd_path.path)
                : "Content"
          }
        />
            ...
            {hasText && (
              <div className="mt-2 p-3 bg-app-box/40 rounded-lg border border-app-line/50">
                <div className="flex items-center gap-2 mb-2">
                  <TextAa className="size-4 text-accent" weight="bold" />
      ...
      {isImage && sidecarUrl ? (
        <div className="size-12 shrink-0 rounded overflow-hidden bg-app-box border border-app-line">
          ...
          <div className="hidden items-center justify-center w-full h-full">
            <Image className="size-5 text-sidebar-inkDull" />
          </div>
        </div>
      ) : (
...
function InstancesTab({ file }: { file: File }) {
  ...
  const getPathDisplay = (sdPath: typeof file.sd_path) => {
    if (sdPath.type === "Physical") {
      return sdPath.path;
    } else if (sdPath.type === "Cloud") {
      return `${sdPath.volume_fingerprint}/${sdPath.path}`;
    } else {
      return `content://${sdPath.content_id}`;
    }
  };
              ...
              <div className="flex items-start gap-2">
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <div className="text-[11px] text-sidebar-inkDull mt-1">
                    {sdPath.type === "Physical" && "Local Device"}
                    {sdPath.type === "Cloud" && "Cloud Storage"}
                    {sdPath.type === "Content" && "Content Addressed"}
                  </div>
                </div>
      ...
      <div className="border-t border-sidebar-line p-2 space-y-2">
        ...
        <div className="flex gap-1">
          <button className="px-2 py-1 text-[10px] font-medium text-sidebar-inkDull hover:text-sidebar-ink bg-app-box/40 hover:bg-app-box/60 rounded-md transition-colors flex items-center gap-1">
            <Sparkle className="size-3" weight="bold" />
            Ask AI
          </button>
    ...
    <div className="no-scrollbar mask-fade-out flex flex-col space-y-5 overflow-x-hidden overflow-y-scroll pb-10">
      ...
      {file.content_identity && (
        <Section title="Content Identity" icon={Fingerprint}>
          ...
          {file.content_identity.mime_type && (
            <InfoRow
              label="MIME Type"
              value={String(file.content_identity.mime_type)}
            />
          )}
        </Section>
      )}
      ...
      {/* Media Info */}
      {file.content_identity?.media_data && (
        <Section title="Media" icon={Palette}>
          <InfoRow
            label="Data"
            value={JSON.stringify(file.content_identity.media_data)}
          />
        </Section>
      )}
      ...
      <Section title="System" icon={DotsThree}>
        <InfoRow label="Entry Kind" value={String(file.kind)} />
        <InfoRow label="Local" value={file.is_local ? "Yes" : "No"} />
        <InfoRow
          label="Instances"
