...
export function useContextMenu(config: ContextMenuConfig): ContextMenuResult {
  ...
  const show = useCallback(
    async (e: React.MouseEvent) => {
      console.log("[useContextMenu] show called", {
        x: e.clientX,
        y: e.clientY,
      });
      e.preventDefault();
      ...
      if (isTauri) {
        ...
        try {
          ...
          if (window.__SPACEDRIVE__?.showContextMenu) {
            ...
            console.warn(
              "[useContextMenu] Tauri context menu handler not found, falling back to web mode"
            );
            setMenuData(visibleItems);
          }
        } catch (err) {
          console.error(
            "[useContextMenu] Failed to show native context menu:",
            err
          );
