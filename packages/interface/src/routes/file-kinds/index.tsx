    ...
    <div className="flex h-full flex-col">
      ...
      <div className="border-app-line border-b px-6 py-4">
        <div className="flex items-center justify-between">
          <div>
            <h1 className="font-bold text-2xl text-ink">File Kinds</h1>
          ...
          <div className="text-right">
            ...
            <div className="text-ink-dull text-xs">Total Files</div>
          </div>
        </div>
      </div>
      ...
      <div className="flex-1 overflow-auto p-3">
        ...
        >
          {stats.map((stat) => {
            const config =
              CONTENT_KIND_CONFIG[stat.name] || CONTENT_KIND_CONFIG.unknown;
            const icon = getIcon(config.iconName, true, null, false);

            return (
              <button
                className="group flex aspect-square flex-col items-center justify-center rounded-lg p-4 transition-colors hover:bg-app-box/50"
                key={stat.name}
                onClick={() => handleKindClick(stat.kind)}
              >
                <img alt={stat.name} className="mb-3 h-16 w-16" src={icon} />
                <div className="w-full text-center">
                  <div className="mb-1 font-medium text-ink text-sm capitalize">
                    {stat.name}
                  </div>
                  <div className="text-ink-dull text-xs">
                    {formatFileCount(Number(stat.file_count))}{" "}
                    {Number(stat.file_count) === 1 ? "file" : "files"}
                  </div>
                </div>
              </button>
            );
