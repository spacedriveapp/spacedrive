  ...
  return useContextMenu({
    items: [
      ...
      },
      {
        ...
        keybind: "⌘O",
        condition: () =>
          !!file && (file.kind === "Directory" || file.kind === "File"),
      },
      {
        type: "submenu",
        icon: ArrowSquareOut,
      ...
      { type: "separator" },
      {
        ...
        condition: () =>
          !!file && selected && selectedFiles.length === 1 && !hasVirtualFiles,
      },
      {
        icon: FolderPlus,
        label: "New Folder",
      ...
      },
      {
        ...
        condition: () =>
          !!currentPath && selectedFiles.length > 0 && !hasVirtualFiles,
      },
      { type: "separator" },
      {
        icon: Copy,
        ...
        submenu: [
          {
            ...
            condition: () =>
              !!file && !file.sidecars?.some((s) => s.kind === "thumbstrip"),
          },
          {
            icon: Sparkle,
            label: "Generate Blurhash",
          ...
          if (confirm(message)) {
            ...
            try {
              ...
              if (
                ...
                console.log("Delete needs confirmation:", result);
                alert("Delete confirmation UI not implemented yet");
              } else if (
                result &&
                typeof result === "object" &&
                "job_id" in result
              ) {
                console.log("Delete job started:", result.job_id);
              }
            } catch (err) {
