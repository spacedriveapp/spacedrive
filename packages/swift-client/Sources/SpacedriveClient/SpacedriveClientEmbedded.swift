...
public class SpacedriveClient {
    ...
    internal func execute<Request: Codable, Response: Codable>(
        ...
        do {
            ...
            let responseJson = try await embeddedCore.sendMessage(requestJson, dataDirectory: dataDirectory)
                ...
                do {
                    ...
                    embeddedCore.startEventListener { [weak self] (eventJson: String) in
                        ...
                        do {
                            ...
                            if eventTypes.isEmpty || self?.shouldIncludeEvent(event, in: eventTypes) == true {
                                continuation.yield(event)
                            }
                        } catch {
                            continuation.finish(throwing: SpacedriveError.serializationError("Failed to decode event: \(error)"))
                        }
                    }
                } catch {
