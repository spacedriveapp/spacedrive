...
export type ActiveJobItem = {
	...
	action_context: ActionContextInfo | null;
};

export type ActiveJobsInput = Record<string, never>;

export type ActiveJobsOutput = {
	jobs: ActiveJobItem[];
...
export type ApfsVolumeInfo = {
	...
	snapshot_mount_point: string | null;
...
export type CreateTagInput = {
	 ...
	 */
	is_organizational_anchor: boolean | null;
 ...
 */
export type DataVolumeSnapshot = {
	entries_synced: {[key in string]: number};
	entries_by_device: {[key in string]: DeviceMetricsSnapshot};
	bytes_sent: number;
	bytes_received: number;
	last_sync_per_peer: {[key in string]: string};
	last_sync_per_model: {[key in string]: string};
};
 ...
 */
export type DeviceMetricsSnapshot = {
	device_id: string;
	device_name: string;
...
export type EphemeralCacheStatus = {
	...
	indexing_in_progress?: number | null;
	indexes?: EphemeralIndexInfo[];
};
...
export type ErrorSnapshot = {
	...
	conflicts_resolved_by_hlc: number;
};
...
export type Event =
	...
	| {JobQueued: {job_id: string; job_type: string; device_id: string}}
	| {JobStarted: {job_id: string; job_type: string; device_id: string}}
	| {
			JobProgress: {
				job_id: string;
				job_type: string;
	...
	| {JobCancelled: {job_id: string; job_type: string; device_id: string}}
	| {JobPaused: {job_id: string; device_id: string}}
	| {JobResumed: {job_id: string; device_id: string}}
...
export type File = {
	 ...
	 */
	image_media_data: ImageMediaData | null;
	video_media_data: VideoMediaData | null;
	audio_media_data: AudioMediaData | null;
	/**
...
export type GenerateSplatInput = {
	entry_uuid: string;
 ...
 */
export type GetSyncActivityOutput = {
	currentState: DeviceSyncState;
	peers: PeerActivity[];
	errorCount: number;
};
...
export type IssueType =
	| {type: 'MissingFromIndex'}
	| {type: 'StaleInIndex'}
	| {type: 'SizeMismatch'}
	| {type: 'ModifiedTimeMismatch'}
	| {type: 'InodeMismatch'}
	| {type: 'ExtensionMismatch'}
	| {type: 'ParentMismatch'}
	| {type: 'KindMismatch'};
...
export type LibraryExportOutput = {
	library_id: string;
	library_name: string;
	export_path: string;
...
export type ListWhisperModelsOutput = {
	models: ModelInfo[];
...
export type LocationAddInput = {
	...
	name: string | null;
	mode: IndexMode;
	job_policies: JsonValue | null;
};

/**
 * Output from location add action dispatch
 */
export type LocationAddOutput = {
	location_id: string;
	path: SdPath;
	name: string | null;
	job_id: string | null;
 ...
 */
export type LocationExportOutput = {
	location_uuid: string;
 ...
 */
export type LocationImportOutput = {
	location_uuid: string;
	location_name: string | null;
	import_path: string;
	stats: ImportStats;
...
export type LocationRescanOutput = {
	location_id: string;
...
export type OperationSnapshot = {
	...
	state_changes_broadcast: number;
	shared_changes_broadcast: number;
	broadcast_batches_sent: number;
	failed_broadcasts: number;
	changes_received: number;
	changes_applied: number;
	changes_rejected: number;
	buffer_queue_depth: number;
	active_backfill_sessions: number;
	backfill_sessions_completed: number;
	backfill_pagination_rounds: number;
...
export type PairGenerateOutput = {
	code: string;
	session_id: string;
	expires_at: string;
	/**
...
export type PairingSessionSummary = {
	id: string;
	state: SerializablePairingState;
...
export type PerformanceSnapshot = {
	...
	apply_latency: LatencySnapshot;
	backfill_request_latency: LatencySnapshot;
	state_watermark: string;
	shared_watermark: string;
	watermark_lag_ms: {[key in string]: number};
	hlc_physical_drift_ms: number;
	hlc_counter_max: number;
	db_query_duration: LatencySnapshot;
...
export type RemoteJobsAllDevicesOutput = {
	jobs_by_device: {[key in string]: RemoteJobState[]};
};
...
export type SearchFacets = {
	file_types: {[key in string]: number};
	tags: {[key in string]: number};
	locations: {[key in string]: number};
	date_ranges: {[key in string]: number};
	size_ranges: {[key in string]: number};
};
...
export type SearchFilters = {
	...
	size_range: SizeRangeFilter | null;
...
export type SerializablePairingState =
	| 'Idle'
	| 'GeneratingCode'
	| 'Broadcasting'
	| 'Scanning'
...
export type SpacedropSendInput = {
	device_id: string;
	paths: SdPath[];
	sender: string | null;
};

export type SpacedropSendOutput = {
	job_id: string | null;
...
export type SyncActivityType =
	| {type: 'BroadcastSent'; data: {changes: number}}
	| {type: 'ChangesReceived'; data: {changes: number}}
	| {type: 'ChangesApplied'; data: {changes: number}}
	| {type: 'BackfillStarted'}
	| {type: 'BackfillCompleted'; data: {records: number}}
	| {type: 'CatchUpStarted'}
	| {type: 'CatchUpCompleted'};

/**
 * A logged sync event
 */
export type SyncEventLog = {
	id: number | null;
	timestamp: string;
	device_id: string;
...
export type SyncStateSnapshot = {
	current_state: DeviceSyncState;
	state_entered_at: string;
	uptime_seconds: number;
	state_history: StateTransition[];
	total_time_in_state: [DeviceSyncState, number][];
	transition_count: [[DeviceSyncState, DeviceSyncState], number][];
};
...
export type Tag = {
	 ...
	 * Advanced capabilities
	 */
	is_organizational_anchor: boolean;
	privacy_level: PrivacyLevel;
	search_weight: number;
	/**
	 * Compositional attributes
	 */
	attributes: {[key in string]: JsonValue};
...
export type VolumeAddCloudInput = {
	service: CloudServiceType;
	display_name: string;
	config: CloudStorageConfig;
};

export type VolumeAddCloudOutput = {
	fingerprint: VolumeFingerprint;
	volume_name: string;
	service: CloudServiceType;
};
 ...
 */
export type VolumeIndexingSuggestion = {
...
export type CoreAction =
	| {
			type: 'core.ephemeral_reset';
			input: EphemeralCacheResetInput;
			output: EphemeralCacheResetOutput;
	  }
	| {type: 'core.reset'; input: ResetDataInput; output: ResetDataOutput}
	| {
			type: 'libraries.create';
			input: LibraryCreateInput;
			output: LibraryCreateOutput;
	  }
	| {
			type: 'libraries.delete';
			input: LibraryDeleteInput;
			output: LibraryDeleteOutput;
	  }
	| {
			type: 'libraries.open';
	...
	| {
			type: 'models.whisper.delete';
			input: DeleteWhisperModelInput;
			output: DeleteWhisperModelOutput;
	  }
	| {
			type: 'models.whisper.download';
			input: DownloadWhisperModelInput;
			output: DownloadWhisperModelOutput;
	  }
	| {
			type: 'network.device.revoke';
			input: DeviceRevokeInput;
			output: DeviceRevokeOutput;
	  }
	| {
	...
	| {
			type: 'network.pair.generate';
			input: PairGenerateInput;
			output: PairGenerateOutput;
	  }
	| {type: 'network.pair.join'; input: PairJoinInput; output: PairJoinOutput}
	| {
			type: 'network.spacedrop.send';
			input: SpacedropSendInput;
			output: SpacedropSendOutput;
	  }
	| {
			type: 'network.start';
			input: NetworkStartInput;
			output: NetworkStartOutput;
	  }
...
export type LibraryAction =
	...
	| {
			type: 'indexing.verify';
			input: IndexVerifyInput;
			output: IndexVerifyOutput;
	  }
	...
	| {
			type: 'libraries.export';
			input: LibraryExportInput;
			output: LibraryExportOutput;
	  }
	| {
			type: 'libraries.rename';
			input: LibraryRenameInput;
			output: LibraryRenameOutput;
	  }
	| {
			type: 'locations.add';
			input: LocationAddInput;
			output: LocationAddOutput;
	  }
	| {
			type: 'locations.enable_indexing';
			input: EnableIndexingInput;
			output: EnableIndexingOutput;
	  }
	| {
			type: 'locations.export';
			input: LocationExportInput;
			output: LocationExportOutput;
	  }
	| {
			type: 'locations.import';
			input: LocationImportInput;
			output: LocationImportOutput;
	  }
	| {
			type: 'locations.remove';
			input: LocationRemoveInput;
			output: LocationRemoveOutput;
	  }
	| {
			type: 'locations.rescan';
			input: LocationRescanInput;
			output: LocationRescanOutput;
	  }
	| {
			type: 'locations.triggerJob';
			input: LocationTriggerJobInput;
			output: LocationTriggerJobOutput;
	  }
	| {
			type: 'locations.update';
			input: LocationUpdateInput;
			output: LocationUpdateOutput;
	  }
	| {
			type: 'media.ocr.extract';
			input: ExtractTextInput;
			output: ExtractTextOutput;
	  }
	| {
			type: 'media.proxy.generate';
			input: GenerateProxyInput;
			output: GenerateProxyOutput;
	  }
	| {
			type: 'media.speech.transcribe';
			input: TranscribeAudioInput;
			output: TranscribeAudioOutput;
	  }
	| {
			type: 'media.splat.generate';
			input: GenerateSplatInput;
			output: GenerateSplatOutput;
	  }
	| {type: 'media.thumbnail'; input: ThumbnailInput; output: JobReceipt}
	| {
			type: 'media.thumbnail.regenerate';
			input: RegenerateThumbnailInput;
			output: RegenerateThumbnailOutput;
	  }
	| {
			type: 'media.thumbstrip.generate';
			input: GenerateThumbstripInput;
			output: GenerateThumbstripOutput;
	  }
	...
	| {
			...
			output: SpaceCreateOutput;
	  }
	| {
			type: 'spaces.delete';
			input: SpaceDeleteInput;
			output: SpaceDeleteOutput;
	  }
	| {
	...
	| {
			type: 'spaces.delete_item';
			input: DeleteItemInput;
			output: DeleteItemOutput;
	  }
	| {
			type: 'spaces.reorder_groups';
			input: ReorderGroupsInput;
			output: ReorderOutput;
	  }
	| {
			type: 'spaces.reorder_items';
	...
	| {
			...
			output: SpaceUpdateOutput;
	  }
	| {
			type: 'spaces.update_group';
	...
	| {
			type: 'volumes.add_cloud';
			input: VolumeAddCloudInput;
			output: VolumeAddCloudOutput;
	  }
	| {
			type: 'volumes.index';
			input: IndexVolumeInput;
			output: IndexVolumeOutput;
	  }
	| {
			type: 'volumes.refresh';
			input: VolumeRefreshInput;
			output: VolumeRefreshOutput;
	  }
	| {
			type: 'volumes.remove_cloud';
			input: VolumeRemoveCloudInput;
			output: VolumeRemoveCloudOutput;
	  }
	| {
			type: 'volumes.speed_test';
			input: VolumeSpeedTestInput;
			output: VolumeSpeedTestOutput;
	  }
	| {
			type: 'volumes.track';
			input: VolumeTrackInput;
			output: VolumeTrackOutput;
	  }
	| {
			type: 'volumes.untrack';
			input: VolumeUntrackInput;
			output: VolumeUntrackOutput;
	  };
...
export type CoreQuery =
	| {
			...
			output: EphemeralCacheStatus;
	  }
	| {
			type: 'core.events.list';
	...
	| {
			...
			input: RemoteJobsAllDevicesInput;
			output: RemoteJobsAllDevicesOutput;
	  }
	| {
			type: 'jobs.remote.for_device';
			input: RemoteJobsForDeviceInput;
			output: RemoteJobsForDeviceOutput;
	  }
	| {type: 'libraries.list'; input: ListLibrariesInput; output: [LibraryInfo]}
	| {
			type: 'models.whisper.list';
			input: ListWhisperModelsInput;
			output: ListWhisperModelsOutput;
	  }
	| {
			type: 'network.devices.list';
			input: ListPairedDevicesInput;
			output: ListPairedDevicesOutput;
	  }
	| {
			type: 'network.pair.status';
			input: PairStatusQueryInput;
			output: PairStatusOutput;
	  }
	| {
			type: 'network.status';
			input: NetworkStatusQueryInput;
...
export type LibraryQuery =
	...
	| {
			type: 'files.alternate_instances';
			input: AlternateInstancesInput;
			output: AlternateInstancesOutput;
	  }
	| {type: 'files.by_id'; input: FileByIdQuery; output: File}
	| {type: 'files.by_path'; input: FileByPathQuery; output: File}
	| {
			type: 'files.content_kind_stats';
			input: ContentKindStatsInput;
			output: ContentKindStatsOutput;
	  }
	| {
			type: 'files.directory_listing';
			input: DirectoryListingInput;
			output: DirectoryListingOutput;
	  }
	| {
			type: 'files.media_listing';
			input: MediaListingInput;
			output: MediaListingOutput;
	  }
	| {
			type: 'files.unique_to_location';
			input: UniqueToLocationInput;
			output: UniqueToLocationOutput;
	  }
	...
	| {
			type: 'locations.list';
			input: LocationsListQueryInput;
			output: LocationsListOutput;
	  }
	| {
			type: 'locations.suggested';
			input: SuggestedLocationsQueryInput;
			output: SuggestedLocationsOutput;
	  }
	| {
			type: 'locations.validate_path';
			input: ValidateLocationPathInput;
			output: ValidateLocationPathOutput;
	  }
	...
	| {
			type: 'spaces.get_layout';
			input: SpaceLayoutQueryInput;
			output: SpaceLayout;
	  }
	| {
			type: 'spaces.list';
			input: SpacesListQueryInput;
			output: SpacesListOutput;
	  }
	| {
			type: 'sync.activity';
			input: GetSyncActivityInput;
			output: GetSyncActivityOutput;
	  }
	| {
			type: 'sync.eventLog';
			input: GetSyncEventLogInput;
			output: GetSyncEventLogOutput;
	  }
	| {
			type: 'sync.metrics';
			input: GetSyncMetricsInput;
			output: GetSyncMetricsOutput;
	  }
	| {type: 'tags.search'; input: SearchTagsInput; output: SearchTagsOutput}
	| {type: 'test.ping'; input: PingInput; output: PingOutput}
	| {
			type: 'volumes.list';
			input: VolumeListQueryInput;
