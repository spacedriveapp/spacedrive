...
describe("useNormalizedQuery - Bulk Moves Integration", () => {
  ...
  beforeAll(async () => {
    ...
    (client as any).subscriptionManager.createSubscription = function (
      ...
      const wrappedCallback = (event: any) => {
        ...
        console.log("[TS] 🔔 Event received:", JSON.stringify(event, null, 2));
        callback(event);
      };
  ...
  test("should update cache when moving 20 files from subfolder to root", async () => {
    ...
    console.log("[TS] Initial root file count:", initialRootData.files.length);
    console.log(
    ...
    const contentAddressedFiles = initialSubfolderData.files.filter(
      (f: any) =>
        f.kind === "File" && f.sd_path?.Content && f.alternate_paths?.length > 0
    );
    ...
    for (const fileName of fileNames) {
      await rename(join(subfolderPath, fileName), join(rootPath, fileName));
    }
    ...
    for (const fileName of fileNames) {
      ...
      if (fileInRoot && !fileInSubfolder && originalFile) {
        ...
        if (fileInRoot.sd_path?.Content) {
          ...
          expect(fileInRoot.alternate_paths.length).toBeGreaterThan(0);
          ...
          expect(fileInRoot.sd_path.Physical.path).toContain(rootPath);
          expect(fileInRoot.sd_path.Physical.path).not.toContain("bulk_test");
          expect(fileInRoot.sd_path.Physical.path).toContain(fileName);
        }
        ...
        console.error(`[TS] ❌ File ${fileName} verification failed:`, {
          ...
          originalType: originalFile?.sd_path?.Content ? "Content" : "Physical",
        });
    ...
    console.log("[TS]   - Physical-only files moved:", physicalOnlyMovedCount);
    ...
    console.log("[TS] ✓ Bulk file move detected and cache updated correctly");
    console.log("[TS] ✓ All files have correct paths and preserved UUIDs");
    console.log("[TS] ✓ No duplicates found");
  }, 45_000); // 45s timeout for bulk operations
});
