...
export type P2PState = { node: { [key in RemoteIdentity]: PeerStatus }; libraries: ([string, { [key in RemoteIdentity]: PeerStatus }])[]; self_peer_id: PeerId; self_identity: RemoteIdentity; config: ManagerConfig; manager_connected: { [key in PeerId]: RemoteIdentity }; manager_connections: PeerId[]; dicovery_services: { [key in string]: { [key in string]: string } | null }; discovery_discovered: { [key in string]: { [key in RemoteIdentity]: [PeerId, { [key in string]: string }, string[]] } }; discovery_known: { [key in string]: RemoteIdentity[] } }
