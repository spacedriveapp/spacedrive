...
export function useLibraryQuery<K extends LibraryQueryKey>(
	...
	if (!libraryId) throw new Error(`Attempted to do library query with no library set!`);
	// @ts-expect-error
	return rspc.useQuery([key[0], { library_id: libraryId, arg: key[1] ?? null }], options);
}
...
export function useLibraryMutation<K extends LibraryMutationKey>(
	...
	return _useMutation<LibraryMutationResult<K>, RSPCError, LibraryMutationArgs<K>>(
		async (data) =>
			await ctx.client.mutation([key, { library_id: libraryId ?? '', arg: data ?? null }]),
		{
			...options,
