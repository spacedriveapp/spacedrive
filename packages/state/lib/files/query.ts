...
export function useFileExplorer(initialPath = '/', initialLocation: number | null = null) {
  ...
  const { data: location } = useQuery(['sys_get_locations', { id: locationId }], () => {
    return bridge('sys_get_locations', { id: locationId });
  });
  ...
  const { data: files } = useQuery(['lib_explore_path', path], async () => {
    return await bridge('lib_explore_path', { limit: fileState.row_limit, path });
  });
