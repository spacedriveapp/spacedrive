...
\begin{itemize}[noitemsep, topsep=0pt]
 ...
 \item \textbf{Safe \& Predictable Operations: The Transactional Action System} (Section~\ref{sec:action-system}): A system that ensures all file operations are safe, transparent, and guaranteed to complete. Every change to the VDFS is treated as a transaction that must be simulated and approved before execution, leveraging the complete VDFS index to run in-memory simulations of proposed actions with precise previews of outcomes, including conflicts or storage issues.

 \item \textbf{Resilient Synchronization: Domain-Separated Consistency} (Section~\ref{sec:library-sync}): A pragmatic synchronization model that avoids the complexity of traditional distributed consensus by separating data into distinct domains, each with its own simple, tailored resolution strategy. The filesystem index treats each device as authority for its own files, user metadata is merged using intuitive rules, and file content transfers are handled as explicit, user-initiated jobs.

 \item \textbf{Intelligent Agency: The AI-Native Architecture} (Section~\ref{sec:ai-native}): An architecture where an AI agent can proactively observe, understand, and act upon the user's data space in a collaborative way. The AI follows an "Observe, Orient, Act" loop, using the VDFS index and rich sidecar data to analyze patterns and generate the same safe, previewable Actions a human would, ensuring the user always remains in control.
\end{itemize}
