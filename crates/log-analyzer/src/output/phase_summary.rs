...
pub fn generate_phase_summary(analyzer: &LogAnalyzer, phase_duration_secs: u64) -> Result<String> {
	...
	output.push_str(&format!("# Aggregates log activity into {}s time windows\n", phase_duration_secs));
	...
	for phase_idx in 0..num_phases {
		let phase_start = start_time + chrono::Duration::seconds((phase_idx * phase_duration_secs) as i64);
		let phase_end = start_time + chrono::Duration::seconds(((phase_idx + 1) * phase_duration_secs) as i64);
		...
		if warning_count > 0 || error_count > 0 {
			output.push_str(&format!(", {} warnings, {} errors", warning_count, error_count));
		}
		output.push_str(")\n\n");
		...
		for (&template_id, &count) in &sorted {
			...
			let module_base = template.module.split("::").take(3).collect::<Vec<_>>().join("::");
			by_module.entry(module_base).or_insert_with(Vec::new).push((template_id, count));
		}
		...
		for (&template_id, &count) in sorted.iter().take(10) {
			let template = templates.iter().find(|t| t.id == template_id).unwrap();
			let module_short = template.module.split("::").last().unwrap_or(&template.module);
