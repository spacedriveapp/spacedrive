...
async fn encrypt() {
	...
	let encryptor = StreamEncryptor::new(master_key, header.nonce, header.algorithm).unwrap();
...
async fn decrypt() {
	...
	let decryptor = StreamDecryptor::new(master_key, header.nonce, header.algorithm).unwrap();
