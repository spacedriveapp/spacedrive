...
function JobGroup({ data, clearJob }: JobGroupProps) {
	...
	const tasks = useMemo(() => getTotalTasks(data.childJobs), [data.childJobs]);
	...
	return <ul className={clsx(`relative overflow-hidden`)}>
		...
		>
			<div className="flex">
				<img
					src={Folder}
					className={clsx(
						'relative left-[-2px] top-2 z-10 mr-3 h-6 w-6'
					)}
				/>
