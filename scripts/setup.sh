...
case "$(uname)" in
  ...
  "Linux") # https://github.com/tauri-apps/tauri-docs/blob/dev/docs/guides/getting-started/prerequisites.md#setting-up-linux
    if has apt-get; then
      ...
      # Tauri dependencies
      set -- "$@" build-essential curl wget file patchelf openssl libssl-dev libgtk-3-dev librsvg2-dev libwebkit2gtk-4.0-dev libgtksourceview-3.0-dev libayatana-appindicator3-dev libglib2.0-dev libc6 libgtk-4-dev libgtk-4-1
