...
export async function tauriUpdaterKey(nativeDeps) {
  ...
  try {
    ...
    await exec(
      `pnpm exec tauri signer generate --ci -w ${quote}${privateKeyPath}${quote}`
    );
    [publicKey, privateKey] = await readKeys();
    if (!(publicKey && privateKey)) throw new Error("Empty keys");
  }
...
export async function patchTauri(root, nativeDeps, targets, args) {
  ...
  if (osType === "Linux") {
    ...
    tauriPatch.bundle.resources = {
      ...
      [path.join(nativeDeps, "models", "yolov8s.onnx")]:
        "./models/yolov8s.onnx",
    };
  }
  ...
  if (osType === "Darwin") {
    ...
    if (macOSMinimumVersion) {
      ...
      throw new Error(
        "No minimum macOS version detected, please review tauri.conf.json"
      );
    }
  }
