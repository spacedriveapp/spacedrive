...
To integrate with Spacedrive's account system, TODO

https://linear.app/spacedriveapp/issue/ENG-1758/p2p-🤝-spacedrive-accounts
...
I've [seen people online](https://medium.com/@andrejtaneski/using-mdns-from-a-docker-container-b516a408a66b) exposing the mDNS daemon of the host machine into the container, and it's possible we could look into doing something similar, however i'm not sure if [`mdns-sd`](https://docs.rs/mdns-sd) uses the OS's daemon and if `avahi` is used by all host Linux distributions we want to support.
...
This allows the administrator to use the `-p 7373:7373` flag when running the container to expose the P2P port to the host machine. This can then be paired with manually entering the IP address and port of the node into the P2P settings of the other node and a connection can be established. This definitely suboptimal but it acts as a workaround for the time being.

This issued is tracked as [ENG-1343](https://linear.app/spacedriveapp/issue/ENG-1343/docker-support-for-p2p).

## Problems with mobile

mDNS discovery does not work on mobile at the moment but the rest of the networking stack should be fine.

We patch [`if-watch`](https://docs.rs/if-watch) using the fork [spacedriveapp/if-watch](https://github.com/spacedriveapp/if-watch). This fork basically implements a *no-op* for mobile so that the core is able to compile.

This issued is tracked as [ENG-1108](https://linear.app/spacedriveapp/issue/ENG-1108/mdns-working-on-ios).

## Problems on Linux

It was reported on Discord that opening Spacedrive would cause excessive network activity. At the time I came the conclusion this might be a bug with the mDNS system but I don't actually recall how I came to that conclusion.

I was unable to reproduce the issue on macOS.

This issued is tracked as [ENG-1319](https://linear.app/spacedriveapp/issue/ENG-1319/excessive-mdns-pings).
