...
<Info>
  ...
  in core/tests/sync_integration_test.rs
</Info>
...
<Info>
  This sync protocol is tested in `test_sync_tag_shared_hlc_based` in
  core/tests/sync_integration_test.rs
</Info>
...
<Info>
  HLC conflict resolution is tested in
  `test_concurrent_tag_updates_hlc_conflict_resolution` in
  core/tests/sync_integration_test.rs
</Info>
...
-- Peer acknowledgment tracking for pruning
...
impl Syncable for YourModel {
    ...
    fn foreign_key_mappings() -> Vec<FKMapping> {
        vec![
            ...
            // Use new_nullable for optional FKs (circular dependencies)
            FKMapping::new_nullable("optional_ref_id", "other_table"),
        ]
    }
}
...
<Info>
  The complete sync infrastructure is validated in
  `test_sync_infrastructure_summary` in core/tests/sync_integration_test.rs
</Info>

### Creating a Location

<Info>
  Location sync with entries is tested in `test_sync_entry_with_location` in
  core/tests/sync_integration_test.rs
</Info>
...
<Info>
  Transitive sync is tested in `test_sync_transitive_three_devices` in
  core/tests/sync_integration_test.rs
</Info>
...
<Info>
  ...
  `test_sync_backfill_includes_pre_sync_data` in
  core/tests/sync_integration_test.rs
</Info>
...
<Info>
  Watermark-based reconnection sync is tested in
  `test_watermark_reconnection_sync` in core/tests/sync_integration_test.rs
</Info>
...
<Info>
  Connection state tracking is tested in `test_connection_state_tracking` in
  core/tests/sync_integration_test.rs
</Info>
...
<Info>See `core/tests/sync_integration_test.rs` for the full test suite.</Info>
