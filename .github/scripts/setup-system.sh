...
if [ $DISTRO = "Darwin" ]; then
      ...
      echo "ffmpeg v5.0.1 has been installed and is now being used on your system."
        ...
        if ! which cargo &> /dev/null; then
                echo "Rust was not detected on your system. Ensure the 'rustc' and 'cargo' binaries are in your \$PATH." && curl https://sh.rustup.rs -sSf | sh -s -- -y  && source $HOME/.cargo/env
        fi
        sudo zypper up
