...
public func setTitlebarStyle(window: NSWindow, fullScreen: Bool) {
  // this results in far less visual artifacts if we just manage it ourselves (tha native taskbar re-appears when fullscreening/un-fullscreening)
  ...
  if fullScreen { // fullscreen, give control back to the native OS
    ...
    // this MUST be hidden while fullscreen as macos has a unique dropdown bar for that, and it's far easier to just let that do its thing
