...
export function openFilePath(library: string, id: number) {
	return invoke()<OpenFilePathResult[]>("open_file_path", { library, ids: [id] })
}

export function getFilePathOpenWithApps(library: string, id: number) {
	return invoke()<OpenWithApplication[]>("get_file_path_open_with_apps", { library, ids: [id] })
}

export function openFilePathWith(library: string, id: number, withUrl: string) {
	return invoke()<null>("open_file_path_with", { library, fileIdsAndUrls: [[id, withUrl]] })
}

export type OpenWithApplication = { t: "File", c: { id: number, name: string, url: string } } | { t: "Error", c: [number, string] }
export type OpenFilePathResult = { t: "NoLibrary" } | { t: "NoFile", c: number } | { t: "OpenError"; c: [number, string] } | { t: "AllGood" }
