...
export default function FileItem(props: Props) {
  return (
    <div className="inline-block text-center w-[100px] h-[100px] mb-6">
      ...
      {props.folder ? (
        <img className="w-[75px] h-[75px] margin-auto inline-block" src="svg/folder.svg" />
      ) : (
        <div className="w-[65px] m-auto rounded-md h-[80px] relative">
          ...
          <div className="absolute flex flex-col items-center justify-center w-full h-full">
            ...
            />
            <span className="mt-1 text-xs font-bold text-center uppercase cursor-default text-gray-550">
              {props.format}
            </span>
          </div>
        </div>
      )}
      <p className="mt-2 text-sm font-medium text-gray-300 cursor-default">{props.fileName}</p>
    </div>
  );
}
