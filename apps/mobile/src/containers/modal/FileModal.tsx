...
import { isObject } from '@sd/client';
		...
		<>
					...
					<View style={tw`flex flex-row items-center`}>
						...
						<View style={tw`ml-2`}>
							...
							<View style={tw`flex flex-row mt-2`}>
								...
								<Text style={tw`ml-1 text-xs text-gray-400`}>{data?.extension.toUpperCase()},</Text>
			...
			>
				<ModalScrollView style={tw`flex-1 p-4`}>
					...
					<>
						{/* Temp, we need cas id */}
						{data?.id && <MetaItem title="Unique Content ID" value={data.id.toString()} />}
