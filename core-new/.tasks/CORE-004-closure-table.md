...
To ensure high-performance hierarchical queries (e.g., finding all files in a directory subtree), a Closure Table pattern has been implemented in the database. This pre-calculates all ancestor-descendant relationships.

## Implementation Notes
-   The schema is defined in `src/infrastructure/database/entities/entry_closure.rs`.
-   Query helpers for traversing the hierarchy (e.g., `get_descendants`, `get_ancestors`) are implemented in `src/operations/indexing/hierarchy.rs`.
