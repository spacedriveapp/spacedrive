...
Implemented `SdPath`, the universal addressing system for the VDFS. This enum provides a core abstraction that makes device boundaries transparent, supporting both direct physical paths (`device_id` + `path`) and abstract content-aware paths.
...
-   URI parsing (`SdPath::from_uri`) is implemented to handle `sd://` schemes for both physical and content paths.
