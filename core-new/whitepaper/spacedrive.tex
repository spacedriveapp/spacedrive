...
\begin{itemize}[noitemsep, topsep=0pt]
 \item \textbf{Virtual Distributed File System} (Section~\ref{sec:vdfs}): Holistic view across all devices with content-aware addressing and seamless cross-device operations.
...
Post-mortem analysis of the v1 codebase revealed critical issues. The most significant was a \textbf{"Dual File System Problem,"} with separate, incompatible systems for indexed and non-indexed locations, which fractured the user experience and doubled development effort. The architecture also suffered from a brittle frontend/backend coupling (the \texttt{invalidate\_query} anti-pattern), an over-engineered CRDT-based synchronization system that was never shipped, excessive boilerplate in the job system, and a fragmented networking layer with poor NAT traversal success. Finally, the project relied on critical dependencies that were later abandoned by their creators.

\subsection{Spacedrive v2 as an Architectural Solution}

The v2 architecture directly addresses these challenges. The universal \textbf{SdPath} addressing system eliminates the dual file system problem entirely. A decoupled \textbf{Event Bus} provides robust state propagation, and a simpler domain-separated \textbf{Library Sync} model ensures consistency without CRDT complexity. The new job system reduces boilerplate by over 90\%, while a consolidated networking layer powered by \textbf{Iroh} unifies all P2P communication, dramatically improving reliability and connection speed~\cite{iroh_networking}. The technology stack was also modernized, replacing abandoned libraries with community-trusted alternatives like \textbf{SeaORM}.
...
To ensure that these powerful addressing modes are accessible and ergonomic for clients, APIs, and command-line interfaces, Spacedrive establishes a unified string-based URI scheme. This allows any path, whether physical or content-based, to be represented as a simple, standardized string:
\begin{itemize}[noitemsep, topsep=0pt]
...
\subsubsection{Quantum State: On-Demand Computation}
Spacedrive treats an entry's state not as a static, stored property, but as a \textbf{quantum property that is computed only upon observation}. This design is fundamental to the VDFS's resilience, as it ensures an entry's state is always a true reflection of ongoing system operations, rather than a field that can become stale or inconsistent.
...
The result of this computation is a well-defined \texttt{EntryState} enum, which serves as a clean API contract for the rest of the application. When a client observes an entry in an active state (like \texttt{Processing}), the returned state includes a \texttt{job\_id}. The client can then use this ID to subscribe to a separate, real-time stream of progress updates directly from the Job Manager. This decouples the high-level state from the high-frequency progress data, leading to a highly efficient and scalable UI.
...
Spacedrive elevates ephemeral user actions, like selecting files, into a powerful organizational tool through \textbf{User-Managed Collections}. This architecture provides the persistence and recall features that traditional file managers lack.
...
\textbf{Small files} (<100KB) receive full BLAKE3 hashing for perfect accuracy.
...
\item \textbf{Proven Performance}: 8,500 files/sec indexing \textbullet{} 55ms keyword search \textbullet{} 95ms semantic search \textbullet{} Reliable P2P connectivity via Iroh~\cite{iroh_networking}
...
% --- APPENDIX ---
\appendix
\section{Implementation Blueprint: SdPath Refactor}
\label{sec:appendix-sdpath-refactor}

The following provides a detailed, actionable blueprint for refactoring the \texttt{SdPath} struct into the content-aware enum described in Section~\ref{sec:vdfs}. This plan is intended to guide an automated or human developer through the necessary codebase modifications.

\subsection{PathResolver Integration}
The \texttt{PathResolver} is a core service and should be integrated into the application's central context.
\begin{itemize}
    \item \textbf{Location:} Create the new resolver at \texttt{src/operations/indexing/path\_resolver.rs}. The existing \texttt{PathResolver} struct in that file, which only handles resolving \texttt{entry\_id} to a \texttt{PathBuf}, should be merged into this new, more powerful service.
    \item \textbf{Integration:} An instance of the new \texttt{PathResolver} should be added to the \texttt{CoreContext} in \texttt{src/context.rs} to make it accessible to all actions and jobs.
    \item \textbf{Cost Function Parameters:} The "optimal path resolution" should be guided by a cost function. The implementation should prioritize sources based on the following, in order:
        \begin{enumerate}
            \item Is the source on the local device? (lowest cost)
            \item What is the network latency to the source's device? (from the \texttt{NetworkingService})
            \item What is the benchmarked speed of the source's volume? (from the \texttt{VolumeManager})
        \end{enumerate}
\end{itemize}

\subsection{Impact on the Codebase (Expanded)}
...
\subsubsection{Action and Job Contracts}
The fundamental principle is that \textbf{Actions receive \texttt{SdPath}s, and Jobs resolve them}.

\paragraph{Action Definitions} All action structs that currently accept \texttt{PathBuf} must be changed to accept \texttt{SdPath}.
\begin{lstlisting}[language=Rust, caption={Updating FileCopyAction in src/operations/files/copy/action.rs}]
...
\end{lstlisting}
This pattern applies to \texttt{FileDeleteAction}, \texttt{ValidationAction}, \texttt{DuplicateDetectionAction}, and others.

\paragraph{Job Execution Flow} Any job that operates on files must begin its \texttt{run} method by resolving its \texttt{SdPath} members.
\begin{lstlisting}[language=Rust, caption={Resolving paths at the start of a Job in src/operations/files/copy/job.rs}]
...
\paragraph{Operation Target Validity} Explicit rules must be enforced within jobs for \texttt{SdPath} variants:
\begin{itemize}
    \item \textbf{Destination/Target:} Operations like copy, move, or index require a physical target. The job must ensure the destination \texttt{SdPath} is or resolves to a \texttt{Physical} variant. An attempt to use a \texttt{Content} variant as a final destination is a logical error and should fail.
    \item \textbf{Source:} A source can be a \texttt{Content} variant, as the resolver will find a physical location for it.
\end{itemize}

\subsubsection{API Layer (CLI Commands)}
The CLI command layer must be updated to accept string URIs.
\begin{itemize}
    \item \textbf{File:} \texttt{src/infrastructure/cli/daemon/types/commands.rs}
    \item \textbf{Action:} Change enums like \texttt{DaemonCommand::Copy} to use \texttt{Vec<String>}.
\end{itemize}
\begin{lstlisting}[language=Rust, caption={Updating DaemonCommand in src/infrastructure/cli/daemon/types/commands.rs}]
...
\end{lstlisting}
The command handlers will then parse these string URIs into \texttt{SdPath} enums.

\subsubsection{Copy Strategy and Routing}
The copy strategy logic must be made \texttt{SdPath} variant-aware.
\begin{itemize}
    \item \textbf{File:} \texttt{src/operations/files/copy/routing.rs}
    \item \textbf{Action:} The \texttt{CopyStrategyRouter::select\_strategy} function must be refactored to resolve source and destination paths first, then compare the \texttt{device\_id} of the resulting \texttt{Physical} paths to select the correct strategy.
    \item \textbf{File:} \texttt{src/operations/files/copy/strategy.rs}
    \item \textbf{Action:} Strategy implementations like \texttt{LocalMoveStrategy} must be modified to only accept resolved, physical paths, enforcing safety by matching the \texttt{SdPath::Physical} variant.
\end{itemize}
