...
In today's digital landscape, the average professional---from designers and photographers to engineers and researchers---manages files across 4-6 devices, multiple cloud services, and countless applications. This fragmentation creates a productivity crisis: knowledge workers spend 25\% of their workweek searching for information across fragmented systems~\cite{atlassian2025}, with creative professionals particularly impacted by media asset sprawl. Storage is wasted on duplicates, and critical data remains vulnerable to loss. Existing solutions force users to choose between convenience (cloud centralization) and control (local storage), with no integrated approach that respects both needs.

\subsection*{The Spacedrive Solution}
Spacedrive implements a Virtual Distributed File System (VDFS) designed to provide a single interface for managing digital assets across disparate storage locations. Rather than moving files to a central location, Spacedrive creates an intelligent layer above existing storage that provides:
...
\item Reduce storage consumption through automated deduplication
...
\item Meet compliance requirements with detailed audit trails
\end{itemize}

\subsection*{Technology Advantages}
Spacedrive's architecture delivers enterprise-grade capabilities on consumer hardware, with performance validated through systematic benchmarking (see Section~\ref{tab:performance}):
\begin{itemize}[noitemsep, topsep=0pt]
\item \textbf{Performance}: Sub-100ms search across millions of files (measured: 55ms keyword, 95ms semantic)
\item \textbf{Reliability}: High-reliability peer-to-peer connections powered by the Iroh networking stack
\item \textbf{Efficiency}: ~150MB memory footprint for 1M+ file libraries (measured: 45MB base + 105MB per 1M files)
\item \textbf{Scalability}: From personal use to multi-petabyte deployments
\end{itemize}

\subsection*{Design Context}
The system architecture addresses use cases ranging from individual creative professionals managing large media libraries to enterprises requiring data sovereignty and compliance. This diversity of requirements informed key design decisions including the local-first approach, robust privacy guarantees, and flexible deployment models.

\subsection*{The Path Forward}
Spacedrive improves how users manage their digital assets. It addresses the challenges of data fragmentation, privacy, and intelligent management, providing a robust infrastructure for personal and organizational data management.
...
\begin{keytakeaways}
\begin{itemize}[noitemsep, topsep=0pt]
\item \textbf{The Problem}: Data fragmentation wastes 25\% of the workweek searching for files---especially for creatives managing sprawling media libraries
\item \textbf{Our Solution}: A Virtual Distributed File System (VDFS) that consolidates all your data with AI-native capabilities while files stay in their original locations
...
The proliferation of computing devices and cloud services has intensified what Bergman et al. identified as the "project fragmentation problem"~\cite{bergman_fragmentation_2006}---a state where digital assets are scattered across incompatible ecosystems, each with proprietary APIs, limited interoperability, and platform lock-in. This challenge spans from individual creators managing files across personal devices to enterprises coordinating data across departments, cloud providers, and geographic locations. Whether it's a photographer organizing a portfolio, a design team collaborating on assets, or a corporation managing petabytes of data, the fundamental problem remains: no complete view or consistent management capabilities across the entire data ecosystem.
...
We present Spacedrive, a Virtual Distributed File System (VDFS) that reimagines data management as an integrated, content-aware ecosystem. Unlike traditional file managers that operate on individual devices, Spacedrive creates Libraries---portable, self-contained databases that maintain complete indexes of content across devices and locations. These Libraries automatically synchronize between all connected devices, ensuring users have a complete, real-time view of their entire data space from any device---whether accessing from their phone, laptop, or workstation.
...
\begin{itemize}[noitemsep, topsep=0pt]
 \item \textbf{Virtual Distributed File System} (Section~\ref{sec:vdfs}): Comprehensive view across all devices with content-aware addressing and seamless cross-device operations.
 ...
 \item \textbf{Content Identity System} (Section~\ref{sec:content-identity}): Deduplication and data redundancy protection through complete content tracking.
...
\begin{itemize}[noitemsep, topsep=0pt]
    ...
    \item \textbf{Fragmented Networking Architecture}: The original codebase suffered from multiple, fragmented networking layers---a centralized cloud sync system built separately from an incomplete libp2p implementation for ephemeral file sharing (Spacedrop), with different protocols for sync, file transfer, and device communication. This fragmentation led to reliability issues, code duplication, and a 70\% NAT traversal success rate that made device-to-device communication unpredictable.
...
\begin{itemize}[noitemsep, topsep=0pt]
    \item The consistent \textbf{SdPath} addressing system completely eliminates the dual file system problem. All file operations now work on a single, consistent abstraction regardless of location.
    ...
    \item A \textbf{Consolidated Networking Layer} powered by Iroh brings together all multi-device communication through a single, well-tested framework. Where the original architecture had separate implementations for cloud sync, Spacedrop, and device discovery, v2 uses one Iroh endpoint with protocol multiplexing via ALPN. By leveraging Iroh~\cite{iroh_networking}, a next-generation P2P stack, Spacedrive is engineered for robust NAT traversal designed to achieve significantly higher connection success rates than the ~70\% baseline of older libraries like libp2p. This architectural choice enables rapid connection establishment (measured at 1.8 seconds in benchmarks), persistent device connections, and transparent failover---all while reducing networking code complexity by over 60\%.
    ...
    \item A full-featured \textbf{async GraphQL API} provides a modern, type-safe interface for frontend applications, while a powerful \textbf{CLI} enables scripting and automation of all Spacedrive operations.

    \item The codebase maintains \textbf{over 95\% line coverage} in core components like the indexing engine, action system, and networking layer, with extensive integration tests ensuring reliability and enabling confident refactoring as the system evolves.
\end{itemize}
...
Version 1's monolithic architecture limited community contributions. Version 2's single WASM plugin model enables a vibrant ecosystem while maintaining security and stability. All extensions—from content type handlers to cloud storage providers—run in the same secure sandbox, simplifying development and distribution.
...
Furthermore, the indexing engine supports an \textbf{ephemeral mode}, which creates temporary, in-memory \texttt{Entry} records for files outside any formally managed \texttt{Location}. When a user browses a filesystem directly, these ephemeral entries are generated on-the-fly and presented "inline" alongside any previously indexed entries, creating a seamless and integrated view. This allows users to, for example, tag a file on their desktop directly from the Spacedrive UI without first adding the entire desktop as a permanent location, demonstrating the system's flexibility in managing both curated and transient data. An entry without a parent Location will be persisted in the Library to host the metadata.
...
\textbf{Small files} (<10MB) receive full SHA-256 hashing for perfect accuracy.
\textbf{Large files} use strategic sampling (3x 1MB segments from beginning, middle, and end), reducing a 10GB file hash from 30+ seconds to under 100ms while maintaining 99.9\%+ deduplication accuracy in practice.

The hashing function, $H(x)$, for a file $F$ of size $S_F$ can be more accurately represented as:

\boldmath
$H_{adaptive}(F) = \begin{cases} H(F) & \text{if } S_F < 10 \text{ MB} \\ H(H(C_1) \oplus H(C_2) \oplus H(C_3)) & \text{if } S_F \geq 10 \text{ MB} \end{cases}$
\unboldmath

Where $H(x)$ is the SHA-256 hash function, $C_1$, $C_2$, and $C_3$ are 1MB chunks sampled from the beginning, middle, and end of the file $F$, and $\oplus$ denotes the concatenation of the resulting hashes.
...
This design minimizes bandwidth/costs: e.g., hashing a 10GB file uses ~3MB of ranged data instead of full transfer. Recent OpenDAL enhancements (v0.49, 2025) improve concurrent listing and metadata handling, enabling ~450 files/sec for cloud indexing.
...
// Remote hashing: Use provider hash if available
async fn hash_remote_entry(op: &Operator, entry: &Entry) -> Result<ContentId> {
    let meta = op.stat(&entry.path).await?;
    if let Some(provider_hash) = meta.content_md5() {  // Or etag for S3
        if is_compatible(&provider_hash, meta.content_length()) {
            return Ok(ContentId::from(provider_hash));  // Skip client compute
        }
    }

    // Fallback: Adaptive sample via ranged reads
    ...
    if size < 10 * 1024 * 1024 {
        ...
        let ranges = vec![(0..1_048_576), (size/2..size/2 + 1_048_576), (size - 1_048_576..size)];
        for range in ranges {
            let chunk = op.read_with(&entry.path).range(range).await?;
            hasher.update(&chunk);
        }
    }
    Ok(ContentId::from(hasher.finalize()))
}
\end{lstlisting}
...
\begin{tikzpicture}[
    ...
    \draw[direct, thick] (deviceA2) to[bend right=30] node[below, font=\tiny] {Direct (always preferred)} (deviceB2);
...
The \texttt{SearchJob} itself executes a hybrid temporal-semantic query designed for performance on consumer hardware:

\textbf{Stage 1: Temporal Filtering (FTS5)}: The system first uses SQLite's FTS5 index to perform a high-speed keyword search on filenames, paths, tags, and extracted text content from sidecars[cite: 5509]. This rapidly narrows millions of potential entries down to a small, relevant candidate set, typically in under 55ms[cite: 5605].

\textbf{Stage 2: Semantic Re-ranking}: For queries that are not simple keyword matches, the system computes a vector embedding of the query using a lightweight, locally-run model (e.g., \texttt{all-MiniLM-L6-v2}) [cite: 5510-5511]. It then re-ranks the candidate set from Stage 1 by comparing the query embedding to the pre-computed embeddings of the candidate files. This re-ranking, operating on a small subset of files, adds minimal latency (typically <40ms)[cite: 5605].

This \textbf{Temporal-First, Vector-Enhanced} approach [cite: 5512] provides the power of semantic search with the speed of traditional indexed search, delivering a superior user experience while fitting perfectly into the asynchronous, job-based architecture.
...
\begin{keytakeaways}
\begin{itemize}[noitemsep, topsep=0pt]
\item \textbf{Protocol-Agnostic Layer}: Treat traditional FS protocols (FTP, SMB, NFS) as standard Volumes for seamless indexing and management
\item \textbf{OpenDAL Foundation}: Native support for FTP, SFTP, WebDAV; extensible plugins for SMB/NFS
\item \textbf{Enhanced Legacy Systems}: Add AI search, deduplication, and P2P alternatives without disrupting existing workflows
\end{itemize}
\end{keytakeaways}

Spacedrive's VDFS extends beyond local and cloud storage to integrate traditional file system protocols, providing a unified interface while preserving native access. Rather than replacing protocols like FTP, Samba (SMB/CIFS), NFS, or WebDAV, Spacedrive treats them as indexed Locations or Volumes. This leverages the OpenDAL library for abstracted access, enabling features like content-aware addressing and semantic search over legacy systems (indexing details in Section~\ref{sec:indexing-engine}).

\subsubsection{Supported Protocols and Integration Strategies}
Spacedrive uses OpenDAL to handle diverse storage services uniformly. As of 2025, OpenDAL supports key protocols including FTP/FTPS, SFTP, WebDAV, and HTTP/HTTPS, with ongoing extensions for others (e.g., HDFS). For unsupported protocols like SMB or NFS, Spacedrive falls back to OS-level mounts or custom WASM plugins.
...
\toprule
\textbf{Protocol} & \textbf{OpenDAL Support} & \textbf{Integration Method} & \textbf{Spacedrive Enhancements} \\
\midrule
FTP/FTPS & Native & Direct via credentials; ranged requests for large files & Adaptive hashing for slow links; AI organization of remote files \\
SFTP & Native & SSH-based auth; indexed as secure Volume & End-to-end encryption overlay; redundancy monitoring \\
WebDAV & Native & Plugin for self-hosted (e.g., Nextcloud); treats as Location & Semantic search over shared docs; transactional previews \\
Samba (SMB/CIFS) & Via plugin/OS mount & WASM extension using smb-client; classify as Network Volume & Deduplication across NAS; P2P alternatives for sharing \\
NFS & Via OS mount/plugin & Mount as local Volume; future OpenDAL extension & Real-time watcher for changes; optimal path resolution \\
Others (e.g., AFP, iSCSI) & OS mount & Treat as block-level Volume if file-semantic & Limited; focus on file-level abstraction \\
\bottomrule
\end{tabular}
\caption{Integration of traditional FS protocols in Spacedrive, with methods and value-adds}
\label{tab:protocol-integrations}
\end{table*}

For example, adding an FTP Location creates a virtual Volume with automatic PhysicalClass assignment (e.g., Warm for WAN access). The indexer performs metadata-only scans initially, with on-demand content fetching to minimize bandwidth.

\begin{lstlisting}[language=Rust, caption={Adding a protocol-based Location via OpenDAL}]
// WASM plugin example for FTP integration
let location = LocationManager::add_protocol_location(
    protocol: "ftp",
    host: "ftp.example.com",
    credentials: encrypted_cred_id,
    path: "/remote/docs/",
).await?;

// Use as standard SdPath
let entries = vdfs.list_directory(&location, "/projects/").await?;
\end{lstlisting}

\subsubsection{Benefits and Optimizations}
This approach addresses protocol limitations: e.g., FTP's weak security is mitigated by Spacedrive's E2E encryption; SMB's versioning gaps are filled by audit logs. Performance optimizations include streaming operations (e.g., chunked transfers for NFS) and AI-driven tiering (e.g., warn on hot data over slow FTP).

For modern alternatives, Spacedrive's P2P (via Iroh) can replace insecure protocols like FTP for sharing, using Spacedrop for ephemeral transfers with perfect forward secrecy.

\planned{Full SMB/NFS support via dedicated plugins, enabling enterprise NAS integration with RBAC.}
...
\begin{table*}[ht]
\centering
\caption{Performance benchmarks across storage tiers. Testing conducted on M2 MacBook Pro with 16GB RAM running macOS 14.5. Values represent median of 10 runs with standard filesystem workloads~\cite{spacedrive-benchmarks-2025}.}
...
\quad External USB 3.2 SSD & $6,200 \pm 180$ & files/sec \\
\quad Network Attached Storage (1Gbps) & $3,100 \pm 150$ & files/sec \\
\quad External HDD (USB 3.0) & $1,850 \pm 120$ & files/sec \\
\quad Cloud Storage (S3, parallel) & $450 \pm 75$ & files/sec \\
\quad Cloud Storage (Google Drive) & $280 \pm 50$ & files/sec \\
\quad FTP (Local Network) & $2,200 \pm 100$ & files/sec \\
\quad SFTP (Remote) & $450 \pm 40$ & files/sec \\
\quad WebDAV (Nextcloud) & $380 \pm 35$ & files/sec \\
\midrule
\multicolumn{3}{l}{\textit{Search Latency (1M entries)}} \\
\quad Temporal Search (FTS5) & $55 \pm 3$ & ms \\
\quad Semantic Search (Vector) & $95 \pm 5$ & ms \\
\quad Combined Temporal-Semantic & $110 \pm 6$ & ms \\
\midrule
...
\quad P2P transfer (LAN) & $110 \pm 8$ & MB/s \\
\quad P2P transfer (WAN w/ relay) & $45 \pm 10$ & MB/s \\
\quad Connection establishment & $1.8 \pm 0.2$ & seconds \\
\midrule
...
To achieve full cloud storage support efficiently, Spacedrive leverages OpenDAL (Open Data Access Layer), a Rust-native library providing unified access to storage services:
...
Spacedrive's architecture includes robust recovery mechanisms:
