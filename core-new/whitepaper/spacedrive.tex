...
\begin{itemize}
    \item \textbf{Location:} Create the new resolver at \texttt{src/operations/indexing/path_resolver.rs}. The existing \texttt{PathResolver} struct in that file, which only handles resolving \texttt{entry_id} to a \texttt{PathBuf}, should be merged into this new, more powerful service.
...
\begin{itemize}
    ...
    \item \textbf{Action:} The \texttt{CopyStrategyRouter::select_strategy} function must be refactored to resolve source and destination paths first, then compare the \texttt{device_id} of the resulting \texttt{Physical} paths to select the correct strategy.
