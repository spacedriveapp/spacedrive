...
This design provides several critical advantages: its self-contained nature simplifies backup and migration, and sharing involves sending the complete Library. The Library acts as a complete catalog of your digital life, preserving your organizational work even if the actual files are distributed across multiple devices and locations. From a security perspective, this architecture ensures that each device in a paired group builds and controls its own library database, enabling granular scoping of locations and indexed data to local or select devices, thereby enhancing data sovereignty and limiting exposure.
...
% --- Visualization of CSV metrics ---
\begin{figure*}[ht]
% Left: Phase-by-recipe (Internal SSD)
...
\begin{axis}[
    ...
    xlabel={Recipe},
    ylabel={Files/s},
    symbolic x coords={shape_small,shape_medium,shape_large},
    xticklabels={shape\_small,shape\_medium,shape\_large},
    xtick=data,
    legend style={at={(0.02,0.98)}, anchor=north west, draw=none, fill=none, font=\small},
    ymajorgrids,
]
% Discovery series (Internal SSD only)
\addplot table[
    x=Recipe,
    y=Files_per_s,
    col sep=comma,
    row predicate/.code={%
        ...
        \ifnum\pdfstrcmp{\thisrow{Phase}}{Discovery}=0\relax\else\pgfplotstableuserowfalse\fi
        \ifnum\pdfstrcmp{\thisrow{Hardware}}{Internal SSD (Macintosh HD)}=0\relax\else\pgfplotstableuserowfalse\fi
    }
]{../benchmarks/results/whitepaper_metrics.csv};
\addlegendentry{Discovery}

% Processing series (Internal SSD only)
\addplot table[
    x=Recipe,
    y=Files_per_s,
    col sep=comma,
    row predicate/.code={%
        \pgfplotstableuserowtrue
        \ifnum\pdfstrcmp{\thisrow{Phase}}{Processing}=0\relax\else\pgfplotstableuserowfalse\fi
        \ifnum\pdfstrcmp{\thisrow{Hardware}}{Internal SSD (Macintosh HD)}=0\relax\else\pgfplotstableuserowfalse\fi
    }
]{../benchmarks/results/whitepaper_metrics.csv};
\addlegendentry{Processing}

% Content Identification series (Internal SSD only)
\addplot table[
    x=Recipe,
    y=Files_per_s,
    col sep=comma,
    row predicate/.code={%
        \pgfplotstableuserowtrue
        \ifnum\pdfstrcmp{\thisrow{Phase}}{Content Identification}=0\relax\else\pgfplotstableuserowfalse\fi
        \ifnum\pdfstrcmp{\thisrow{Hardware}}{Internal SSD (Macintosh HD)}=0\relax\else\pgfplotstableuserowfalse\fi
    }
]{../benchmarks/results/whitepaper_metrics.csv};
\addlegendentry{Content Identification}

\end{axis}
\end{tikzpicture}
\subcaption{Internal SSD: Files/s by phase and recipe}
\label{fig:throughput_files_per_s}
\end{minipage}%
\hfill

% Right: Storage medium comparison (shape_large)
\begin{minipage}[t]{.49\textwidth}
\begin{tikzpicture}
\begin{axis}[
    ybar,
    ...
    xlabel={Phase},
    ylabel={Files/s},
    symbolic x coords={Discovery,Processing,Content Identification},
    xtick=data,
    legend style={at={(0.98,0.98)}, anchor=north east, draw=none, fill=none, font=\small},
    ymajorgrids,
]
% Internal SSD series (shape_large)
\addplot table[
    x=Phase,
    y=Files_per_s,
    col sep=comma,
    row predicate/.code={%
        \pgfplotstableuserowtrue
        \ifnum\pdfstrcmp{\thisrow{Hardware}}{Internal SSD (Macintosh HD)}=0\relax\else\pgfplotstableuserowfalse\fi
        \ifnum\pdfstrcmp{\thisrow{Recipe}}{shape_large}=0\relax\else\pgfplotstableuserowfalse\fi
    }
]{../benchmarks/results/whitepaper_metrics.csv};
\addlegendentry{Internal SSD}

% External HDD series (shape_large)
\addplot table[
    x=Phase,
    y=Files_per_s,
    col sep=comma,
    row predicate/.code={%
        \pgfplotstableuserowtrue
        \ifnum\pdfstrcmp{\thisrow{Hardware}}{External HDD (Seagate)}=0\relax\else\pgfplotstableuserowfalse\fi
        \ifnum\pdfstrcmp{\thisrow{Recipe}}{shape_large}=0\relax\else\pgfplotstableuserowfalse\fi
    }
]{../benchmarks/results/whitepaper_metrics.csv};
\addlegendentry{External HDD}

\end{axis}
\end{tikzpicture}
\subcaption{Shape\_large: Internal SSD vs External HDD}
\label{fig:hardware_comparison_files_per_s}
\end{minipage}
