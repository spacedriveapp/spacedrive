...
pub async fn create_location(
    ...
    start_location_indexing(library.clone(), events, location_db_id, location_id, args.path, args.index_mode).await?;
...
async fn start_location_indexing(
    ...
    match library.jobs().dispatch(indexer_job).await {
        Ok(job_handle) => {
            info!("Successfully dispatched indexer job {} for location: {}", 
                  job_handle.id(), path.display());
        ...
        Err(e) => {
            error!("Failed to dispatch indexer job for {}: {}", path.display(), e);
            
            // Update scan state to failed
            if let Err(update_err) = update_location_scan_state(library.clone(), location_db_id, "failed", Some(e.to_string())).await {
            ...
            return Err(LocationError::Other(format!("Failed to start indexing: {}", e)));
        }
    }
...
async fn monitor_indexing_job(
    ...
    info!("Monitoring indexer job {} for location: {}", job_handle.id(), path.display());
    ...
    match job_result {
        Ok(output) => {
            info!("Indexing completed successfully for location: {}", path.display());
            ...
            if let Some(indexer_output) = output.as_indexed() {
                ...
                if let Err(e) = update_location_scan_state(library.clone(), location_db_id, "completed", None).await {
                ...
                info!("Location indexing completed: {} ({} files, {} dirs, {} bytes)", 
                      path.display(), 
                      indexer_output.total_files, 
...
/// Create sample database entries to demonstrate integration
async fn create_sample_database_entries(
    library: Arc<Library>,
    location_id: i32,
    count: u64,
) -> LocationResult<()> {
    info!("Creating {} sample database entries to demonstrate indexer integration", count);

    // Find a device for the path prefix
    ...
    for i in 0..count {
        ...
        let entry_model = entities::entry::ActiveModel {
            ...
            file_count: Set(0), // Files don't contain other files
            created_at: Set(chrono::Utc::now()),
    ...
    info!("Created {} sample entries with user metadata", count);
    Ok(())
}
...
pub async fn list_locations(library: Arc<Library>) -> LocationResult<Vec<entities::location::Model>> {
