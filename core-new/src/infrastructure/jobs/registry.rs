...
impl JobRegistry {
    ...
    pub fn deserialize_dyn_job(&self, name: &str, data: &[u8]) -> JobResult<Box<dyn super::traits::DynJob>> {
        let registration = self.jobs.get(name)
            .ok_or_else(|| JobError::NotFound(format!("Job type '{}' not found", name)))?;

        (registration.deserialize_dyn_fn)(data)
            .map_err(|e| JobError::serialization(format!("Failed to deserialize job '{}': {}", name, e)))
    }
