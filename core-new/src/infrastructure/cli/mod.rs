...
pub async fn run() -> Result<(), Box<dyn std::error::Error>> {
    ...
    tracing_subscriber::fmt()
        .with_env_filter(format!("sd_core_new={},spacedrive_cli={}", log_level, log_level))
        .init();
    
    // Determine data directory
    let data_dir = cli.data_dir.unwrap_or_else(|| {
        PathBuf::from("./spacedrive-cli-data")
    });
    
    // Handle daemon commands first (they don't need Core)
    match &cli.command {
        Commands::Start { foreground } => {
            return handle_start_daemon(data_dir, *foreground).await;
        }
        Commands::Stop => {
    ...
    match cli.command {
        ...
        Commands::Start { .. } | Commands::Stop | Commands::Daemon => {
...
async fn handle_start_daemon(data_dir: PathBuf, foreground: bool) -> Result<(), Box<dyn std::error::Error>> {
    if daemon::Daemon::is_running() {
    ...
    if daemon::Daemon::is_running() {
        ...
        match client.send_command(daemon::DaemonCommand::GetStatus).await {
            Ok(daemon::DaemonResponse::Status(status)) => {
                ...
                println!("   Uptime: {} seconds", status.uptime_secs.to_string().bright_yellow());
                println!("   Active Jobs: {}", status.active_jobs.to_string().bright_green());
        ...
        // Get libraries
        match client.send_command(daemon::DaemonCommand::ListLibraries).await {
        ...
        // Get current library
        match client.send_command(daemon::DaemonCommand::GetCurrentLibrary).await {
...
async fn handle_library_daemon_command(cmd: commands::LibraryCommands) -> Result<(), Box<dyn std::error::Error>> {
    use colored::Colorize;
    ...
    match cmd {
        commands::LibraryCommands::Create { name, path } => {
            ...
            match client.send_command(daemon::DaemonCommand::CreateLibrary { name: name.clone(), path }).await {
        ...
        commands::LibraryCommands::Switch { identifier } => {
            match client.send_command(daemon::DaemonCommand::SwitchLibrary { id: identifier.parse()? }).await {
...
async fn handle_location_daemon_command(cmd: commands::LocationCommands) -> Result<(), Box<dyn std::error::Error>> {
    use colored::Colorize;
    ...
    match cmd {
        commands::LocationCommands::Add { path, name, mode } => {
            ...
            match client.send_command(daemon::DaemonCommand::AddLocation { path: path.clone(), name }).await {
                Ok(daemon::DaemonResponse::LocationAdded { location_id, job_id }) => {
                    println!("✅ Location added successfully!");
                    println!("   Path: {}", path.display().to_string().bright_blue());
                    println!("   Location ID: {}", location_id.to_string().bright_yellow());
                    
                    if !job_id.is_empty() {
                        println!("   Job ID: {}", job_id.chars().take(8).collect::<String>().bright_yellow());
                        
                        // Automatically show brief progress info
                        println!("\n📊 Indexing started...");
                        println!("   To monitor detailed progress, run: {}", "spacedrive job monitor".bright_cyan());
                        ...
                        for _ in 0..10 {  // Check for 10 seconds
                            ...
                            if let Ok(uuid) = job_id.parse::<uuid::Uuid>() {
                                match client.send_command(daemon::DaemonCommand::GetJobInfo { id: uuid }).await {
        ...
        commands::LocationCommands::List => {
            match client.send_command(daemon::DaemonCommand::ListLocations).await {
                Ok(daemon::DaemonResponse::Locations(locations)) => {
                    if locations.is_empty() {
                        println!("📭 No locations found. Add one with: spacedrive location add <path>");
                    } else {
                        use comfy_table::Table;
                        let mut table = Table::new();
        ...
        commands::LocationCommands::Remove { identifier } => {
            ...
            match client.send_command(daemon::DaemonCommand::RemoveLocation { id }).await {
                Ok(daemon::DaemonResponse::Ok) => {
        ...
        commands::LocationCommands::Rescan { identifier, force } => {
            ...
            match client.send_command(daemon::DaemonCommand::RescanLocation { id }).await {
                Ok(daemon::DaemonResponse::Ok) => {
...
async fn handle_job_daemon_command(cmd: commands::JobCommands) -> Result<(), Box<dyn std::error::Error>> {
    use colored::Colorize;
    ...
    match cmd {
        commands::JobCommands::List { status, recent: _ } => {
            ...
            match client.send_command(daemon::DaemonCommand::ListJobs { status: status_filter }).await {
        ...
        commands::JobCommands::Info { id } => {
            match client.send_command(daemon::DaemonCommand::GetJobInfo { id }).await {
                Ok(daemon::DaemonResponse::JobInfo(Some(job))) => {
                    println!("📋 Job Information");
                    println!("   ID: {}", job.id.to_string().bright_yellow());
                    println!("   Name: {}", job.name.bright_cyan());
                    println!("   Status: {}", match job.status.as_str() {
                        "running" => job.status.bright_yellow(),
                        "completed" => job.status.bright_green(),
                        "failed" => job.status.bright_red(),
                        _ => job.status.normal(),
        ...
        commands::JobCommands::Monitor { job_id } => {
            println!("📡 {} - Press Ctrl+C to exit", "Spacedrive Job Monitor".bright_cyan());
            println!("═══════════════════════════════════════════");
            println!();
            ...
            if let Some(ref specific_job_id) = job_id {
                println!("📊 Monitoring job {}...\n", specific_job_id.chars().take(8).collect::<String>().bright_yellow());
            } else {
