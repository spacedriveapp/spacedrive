...
use crate::{Core, infrastructure::database::entities};
...
pub enum DaemonResponse {
    ...
    LocationAdded { location_id: Uuid, job_id: String },
...
async fn handle_command(
    ...
    match cmd {
        ...
        DaemonCommand::ListLibraries => {
            let libraries = core.libraries.list().await;
            let infos: Vec<LibraryInfo> = futures::future::join_all(
                libraries.into_iter().map(|lib| async move {
                    LibraryInfo {
                        id: lib.id(),
                        name: lib.name().await,
            ...
            match core.libraries.create_library(&name, path).await {
                Ok(library) => {
                    ...
                    match device_model.insert(db.conn()).await {
                        Ok(_) => {
                            info!("Registered device {} in new library {}", device.id, library.id());
                        ...
                        Err(e) => DaemonResponse::Error(format!("Failed to register device: {}", e)),
                    }
                }
        ...
        DaemonCommand::AddLocation { path, name } => {
            ...
            if let Some(library) = libraries.first() {
                ...
                {
                    Ok(Some(d)) => d,
                    Ok(None) => return DaemonResponse::Error("Device not registered in database".to_string()),
                    Err(e) => return DaemonResponse::Error(format!("Database error: {}", e)),
                };
                
                // Create location manager
                let location_manager = crate::location::LocationManager::new((*core.events).clone());
                
                // Add location
                match location_manager.add_location(
                    library.clone(),
                    path.clone(),
                    name,
                    device_record.id,
        ...
        DaemonCommand::ListLocations => {
            ...
            if let Some(library) = libraries.first() {
                ...
                match location_manager.list_locations(library.as_ref()).await {
                    Ok(locations) => {
                        let infos: Vec<LocationInfo> = locations.into_iter().map(|loc| {
                            LocationInfo {
                                id: loc.id,
                                name: loc.name,
                                path: loc.path,
        ...
        DaemonCommand::RescanLocation { id } => {
            ...
            if let Some(library) = libraries.first() {
                ...
                match location_manager.rescan_location(library.clone(), id, false).await {
                    Ok(_) => DaemonResponse::Ok,
                    Err(e) => DaemonResponse::Error(e.to_string()),
                }
            } else {
            ...
            if let Some(library) = libraries.first() {
                ...
                if let Some(ref status_str) = status {
                    if status_str == "running" {
                        ...
                        let infos: Vec<JobInfo> = running_jobs.into_iter().map(|j| {
                            JobInfo {
                                id: j.id,
                                name: j.name,
                ...
                match job_manager.list_jobs(status_filter).await {
                    Ok(jobs) => {
                        let infos: Vec<JobInfo> = jobs.into_iter().map(|j| {
                            JobInfo {
                                id: j.id,
                                name: j.name,
                                status: j.status.to_string(),
        ...
        DaemonCommand::GetJobInfo { id } => {
            ...
            if let Some(library) = libraries.first() {
                ...
                match job_manager.get_job_info(id).await {
                    Ok(job) => DaemonResponse::JobInfo(job.map(|j| {
                        JobInfo {
                            id: j.id,
                            name: j.name,
                            status: j.status.to_string(),
...
impl DaemonClient {
    ...
    pub async fn send_command(&self, cmd: DaemonCommand) -> Result<DaemonResponse, Box<dyn std::error::Error>> {
