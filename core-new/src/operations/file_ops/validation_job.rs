...
impl JobHandler for ValidationJob {
    ...
    async fn run(&mut self, ctx: JobContext<'_>) -> JobResult<Self::Output> {
        ctx.log(format!("Starting file validation with mode: {:?}", self.mode));
            ...
            match self.validate_file(file_info, &ctx).await {
                ...
                Err(e) => {
                    issues.push(ValidationIssue {
                        ...
                        suggested_action: Some("Check file permissions and accessibility".to_string()),
                    });
                    ctx.add_non_critical_error(format!(
                        "Failed to validate {}: {}", file_info.path.display(), e
                    ));
                }
            }
        ...
        let error_count = issues.iter().filter(|i| matches!(i.severity, ValidationSeverity::Error | ValidationSeverity::Critical)).count();
        let warning_count = issues.iter().filter(|i| matches!(i.severity, ValidationSeverity::Warning)).count();
        
        ctx.log(format!("Validation completed: {} files validated, {} errors, {} warnings", 
...
impl ValidationJob {
    ...
    async fn collect_files(&self, ctx: &JobContext<'_>) -> JobResult<Vec<FileValidationInfo>> {
        ...
        for target in &self.targets.paths {
            ...
            if let Some(local_path) = target.as_local_path() {
                self.collect_files_recursive(local_path, target, &mut files, ctx).await?;
            }
        }
        ...
        while let Some((current_path, current_sd_path)) = stack.pop() {
            ...
            if metadata.is_file() {
                files.push(FileValidationInfo {
                    ...
                    permissions: std::fs::metadata(&current_path).ok().map(|m| m.permissions()),
                });
            ...
            if current_metadata.len() != file_info.size {
                issues.push(ValidationIssue {
                    ...
                    description: format!(
                        ...
                        file_info.size, current_metadata.len()
                    ),
                ...
                if let Ok(current_metadata) = std::fs::metadata(local_path) {
                    ...
                    if current_permissions.readonly() != expected_permissions.readonly() {
                        issues.push(ValidationIssue {
                            ...
                            suggested_action: Some("Check if permission change was intentional".to_string()),
                        });
                    }
                }
            }
    ...
    async fn validate_integrity(
        ...
        match CasGenerator::generate_cas_id(local_path).await {
            Ok(current_cas_id) => {
                // Here we would compare against the stored CAS ID from the database
                // For now, we just verify that we can generate one
                ctx.log(format!("Generated CAS ID for validation: {}", &current_cas_id[..16]));
            }
            Err(e) => {
                issues.push(ValidationIssue {
                    path: file_info.path.clone(),
    ...
    async fn validate_corruption(
        ...
        if file_info.size > 100 * 1024 * 1024 * 1024 { // 100GB
            issues.push(ValidationIssue {
                path: file_info.path.clone(),
                ...
                "txt" | "md" | "json" | "xml" | "csv" => {
                    ...
                    if let Ok(mut file) = tokio::fs::File::open(local_path).await {
                        ...
                        if let Ok(bytes_read) = file.read(&mut buffer).await {
                            ...
                            if content.iter().any(|&b| b < 32 && b != 9 && b != 10 && b != 13) {
                                issues.push(ValidationIssue {
                                    path: file_info.path.clone(),
                                    issue_type: "SuspiciousContent".to_string(),
                                    severity: ValidationSeverity::Warning,
                                    description: "Text file contains binary data".to_string(),
                                    suggested_action: Some("Verify file type is correct".to_string()),
                                });
                            }
                        }
                    }
        ...
        if let Some(filename) = local_path.file_name().and_then(|n| n.to_str()) {
            ...
            if filename.len() > 255 {
                issues.push(ValidationIssue {
                    ...
                    severity: ValidationSeverity::Warning,
                    description: "Filename is very long and may not be compatible with all systems".to_string(),
