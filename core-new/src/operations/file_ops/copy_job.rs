...
impl JobHandler for FileCopyJob {
    ...
    async fn run(&mut self, ctx: JobContext<'_>) -> JobResult<Self::Output> {
        ...
        ctx.log(format!("Copy operation completed: {} copied, {} failed", 
            copied_count, failed_copies.len()));
...
impl FileCopyJob {
                ...
                match self.copy_local_file(local_source, &dest_path).await {
                    ...
                    Err(e) => {
                        ...
                        ctx.add_non_critical_error(format!(
                            "Failed to copy {}: {}", local_source.display(), e
                        ));
                    }
                }
    ...
    async fn process_cross_device_copies(
        ...
        for source in paths {
            ...
            ctx.add_non_critical_error(format!(
                "Cross-device copy not yet implemented for: {}", source.display()
            ));
...
// Register the job
// In a real implementation, this would be done by the derive macro
// For now, we'll skip this until we implement the proper type erasure