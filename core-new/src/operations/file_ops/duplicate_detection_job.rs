...
impl JobHandler for DuplicateDetectionJob {
    ...
    async fn run(&mut self, ctx: JobContext<'_>) -> JobResult<Self::Output> {
        ctx.log(format!("Starting duplicate detection with mode: {:?}", self.mode));
        
        // Collect all files to scan
        let files_to_scan = self.collect_files(&ctx).await?;
        let total_files = files_to_scan.len();
        
        ctx.log(format!("Found {} files to scan for duplicates", total_files));
        
        // Phase 1: Group by size
        self.group_by_size(&files_to_scan, &ctx, total_files).await?;
...
impl DuplicateDetectionJob {
    ...
    async fn collect_files(&self, ctx: &JobContext<'_>) -> JobResult<Vec<FileInfo>> {
        ...
        for search_path in &self.search_paths.paths {
            ...
            if let Some(local_path) = search_path.as_local_path() {
                self.collect_files_recursive(local_path, search_path, &mut files, ctx).await?;
            }
        }
    ...
    async fn group_by_size(
        ...
        for (index, file) in files.iter().enumerate() {
            ...
            self.size_groups.entry(file.size).or_insert_with(Vec::new).push(file.clone());
    ...
    async fn find_content_duplicates(&self, ctx: &JobContext<'_>) -> JobResult<Vec<DuplicateGroup>> {
        let mut groups = Vec::new();
        let mut processed = 0;
        let total_candidates: usize = self.size_groups.values().map(|files| {
            if files.len() > 1 { files.len() } else { 0 }
        }).sum();
        ...
        for (size, files) in &self.size_groups {
            if files.len() > 1 {
                ...
                // Generate CAS IDs for files with same size
                ...
                for file in files {
                    ctx.progress(Progress::structured(DuplicateProgress {
                        ...
                        potential_savings: groups.iter().map(|g: &DuplicateGroup| g.wasted_space).sum(),
                    ...
                    if let Some(local_path) = file.path.as_local_path() {
                        match CasGenerator::generate_cas_id(local_path).await {
                            Ok(cas_id) => {
                                ...
                                file_with_cas.cas_id = Some(cas_id.clone());
                                hash_groups.entry(cas_id).or_insert_with(Vec::new).push(file_with_cas);
                            }
                            Err(e) => {
    ...
    async fn find_name_size_duplicates(&self, ctx: &JobContext<'_>) -> JobResult<Vec<DuplicateGroup>> {
        let mut groups = Vec::new();
    ...
    async fn find_deep_scan_duplicates(&self, ctx: &JobContext<'_>) -> JobResult<Vec<DuplicateGroup>> {
