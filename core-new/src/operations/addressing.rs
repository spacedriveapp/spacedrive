...
impl PathResolver {
    ...
    pub async fn resolve(
        ...
        match path {
            ...
            SdPath::Content { content_id } => {
                self.resolve_content_path(context, *content_id).await
            }
        }
    }
    ...
    async fn find_content_instances<C: ConnectionTrait>(
        ...
        let content_identity = ContentIdentity::find()
            ...
            .one(db)
            .await?
            .ok_or_else(|| DbErr::RecordNotFound(format!("Content identity not found: {}", content_id)))?;
    ...
    async fn find_content_instances_batch<C: ConnectionTrait>(
        ...
        let content_identities = ContentIdentity::find()
            .filter(content_identity::Column::Uuid.is_in(content_ids.iter().map(|id| Some(*id))))
            .all(db)
            .await?;
        ...
        let entries = Entry::find()
            .filter(entry::Column::ContentId.is_in(db_ids.iter().map(|id| Some(*id))))
            .all(db)
            .await?;
        ...
        let locations = Location::find()
            .filter(location::Column::EntryId.is_in(entry_ids))
            .all(db)
            .await?;
        ...
        let device_map: HashMap<i32, device::Model> = devices
            .into_iter()
            .map(|dev| (dev.id, dev))
            .collect();
        
        // Create reverse map from database ID to UUID
        let id_to_uuid: HashMap<i32, Uuid> = uuid_to_id.iter()
            .map(|(uuid, id)| (*id, *uuid))
            .collect();
    ...
    fn select_optimal_instance(
        ...
        let mut candidates: Vec<(f64, &ContentInstance)> = instances
            ...
            .map(|inst| {
                let cost = self.calculate_instance_cost(
                    inst,
                    current_device_id,
                    device_metrics,
                );
