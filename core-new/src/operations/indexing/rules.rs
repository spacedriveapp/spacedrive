...
impl RuleKind {
    #[must_use]
    pub const fn variant_count() -> usize { 5 }
}
        ...
        globs_str
            ...
            .and_then(|globs| {
                globs
                    .iter()
                    .cloned()
                    .fold(&mut GlobSetBuilder::new(), |builder, glob| builder.add(glob))
                    .build()
...
pub trait MetadataForIndexerRules: Send + Sync + 'static { fn is_dir(&self) -> bool; }
...
impl RulePerKind {
    pub async fn apply(
        ...
        match self {
            Self::AcceptIfChildrenDirectoriesArePresent(children) => accept_dir_for_its_children(source, metadata, children)
                .await
                .map(|accepted| (RuleKind::AcceptIfChildrenDirectoriesArePresent, accepted)),
            Self::RejectIfChildrenDirectoriesArePresent(children) => reject_dir_for_its_children(source, metadata, children)
                .await
                .map(|rejected| (RuleKind::RejectIfChildrenDirectoriesArePresent, rejected)),
            Self::AcceptFilesByGlob(_globs, accept_glob_set) => Ok((RuleKind::AcceptFilesByGlob, accept_by_glob(source, accept_glob_set))),
            Self::RejectFilesByGlob(_globs, reject_glob_set) => Ok((RuleKind::RejectFilesByGlob, reject_by_glob(source, reject_glob_set))),
            Self::IgnoredByGit(base_dir, patterns) => Ok((RuleKind::IgnoredByGit, accept_by_git_pattern(source, base_dir, patterns))),
        }
    }
}

fn accept_by_git_pattern(source: impl AsRef<Path>, base_dir: impl AsRef<Path>, search: &Search) -> bool {
    fn inner(source: &Path, base_dir: &Path, search: &Search) -> bool {
        let relative = match source.strip_prefix(base_dir) { Ok(p) => p, Err(_) => return true };
        let Some(src) = relative.to_str().map(|s| s.as_bytes().into()) else { return false };
        search.pattern_matching_relative_path(src, Some(source.is_dir()), Case::Fold)
...
pub enum RulerDecision { Accept, Reject }
...
impl Clone for IndexerRuler {
    fn clone(&self) -> Self { Self { base: Arc::clone(&self.base), extra: Vec::new() } }
}

impl IndexerRuler {
    #[must_use]
    pub fn new(rules: Vec<IndexerRule>) -> Self { Self { base: Arc::new(rules), extra: Vec::new() } }
    ...
    pub async fn evaluate_path(
        ...
        Ok(if Self::reject_path(Path::new(""), false, &acceptance_per_rule_kind) { RulerDecision::Reject } else { RulerDecision::Accept })
    }
    ...
    pub async fn apply_all(
        ...
        let results = self
            ...
            .try_join()
            .await?;
        Ok(results.into_iter().flatten().fold(HashMap::<_, Vec<_>>::with_capacity(RuleKind::variant_count()), |mut map, (kind, result)| { map.entry(kind).or_default().push(result); map }))
    }

    pub fn extend(&mut self, iter: impl IntoIterator<Item = IndexerRule> + Send) { self.extra.extend(iter); }
    ...
    pub fn rejected_by_accept_glob(acceptance_per_rule_kind: &HashMap<RuleKind, Vec<bool>>) -> bool {
        acceptance_per_rule_kind
            .get(&RuleKind::AcceptFilesByGlob)
            .map_or(false, |accept_rules| accept_rules.iter().all(|accept| !accept))
    }

    pub fn rejected_by_children_directories(acceptance_per_rule_kind: &HashMap<RuleKind, Vec<bool>>) -> bool {
        acceptance_per_rule_kind
            .get(&RuleKind::RejectIfChildrenDirectoriesArePresent)
            .map_or(false, |reject_results| reject_results.iter().any(|reject| !reject))
    }

    pub fn rejected_by_reject_glob(acceptance_per_rule_kind: &HashMap<RuleKind, Vec<bool>>) -> bool {
        acceptance_per_rule_kind
...
impl Serialize for RulePerKind {
    ...
    where S: serde::ser::Serializer {
        use serde::ser;
        match *self {
            Self::AcceptFilesByGlob(ref globs, _) => serializer.serialize_newtype_variant("ParametersPerKind", 0, "AcceptFilesByGlob", globs),
            Self::RejectFilesByGlob(ref globs, _) => serializer.serialize_newtype_variant("ParametersPerKind", 1, "RejectFilesByGlob", globs),
            Self::AcceptIfChildrenDirectoriesArePresent(ref children) => serializer.serialize_newtype_variant("ParametersPerKind", 2, "AcceptIfChildrenDirectoriesArePresent", children),
            Self::RejectIfChildrenDirectoriesArePresent(ref children) => serializer.serialize_newtype_variant("ParametersPerKind", 3, "RejectIfChildrenDirectoriesArePresent", children),
            Self::IgnoredByGit(_, _) => unreachable!("git ignore rules are dynamic and not serialized"),
        }
    }
}
...
impl<'de> Deserialize<'de> for RulePerKind {
    ...
    where D: serde::de::Deserializer<'de> {
        ...
        enum Fields { AcceptFilesByGlob, RejectFilesByGlob, AcceptIfChildrenDirectoriesArePresent, RejectIfChildrenDirectoriesArePresent }
        ...
        impl de::Visitor<'_> for FieldsVisitor {
            ...
            fn visit_str<E: de::Error>(self, value: &str) -> Result<Self::Value, E> {
                Ok(match value {
                    ...
                    "AcceptIfChildrenDirectoriesArePresent" => Fields::AcceptIfChildrenDirectoriesArePresent,
                    "RejectIfChildrenDirectoriesArePresent" => Fields::RejectIfChildrenDirectoriesArePresent,
                    _ => return Err(E::unknown_variant(value, VARIANTS)),
                })
            }
        }
        impl<'de> Deserialize<'de> for Fields {
            fn deserialize<D2: de::Deserializer<'de>>(d: D2) -> Result<Self, D2::Error> { d.deserialize_identifier(FieldsVisitor) }
        }

        struct Visitor<'de> { marker: PhantomData<RulePerKind>, lifetime: PhantomData<&'de ()> }
        impl<'de> de::Visitor<'de> for Visitor<'de> {
            type Value = RulePerKind;
            fn expecting(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result { f.write_str("enum ParametersPerKind") }
        ...
        deserializer.deserialize_enum("ParametersPerKind", VARIANTS, Visitor { marker: std::marker::PhantomData, lifetime: std::marker::PhantomData })
    }
}
...
async fn accept_dir_for_its_children(
    ...
    let mut read_dir = tokio::fs::read_dir(source.as_ref()).await.map_err(|_| Error::RuleParametersRMPDecode(rmp_serde::decode::Error::TypeMismatch(Marker::FixPos(0))))?;
    while let Ok(Some(entry)) = read_dir.next_entry().await {
        let entry_name = match entry.file_name().into_string() { Ok(s) => s, Err(_) => continue };
        if entry.metadata().await.map(|m| m.is_dir()).unwrap_or(false) && children.contains(&entry_name) { return Ok(true); }
    }
    Ok(false)
}
...
async fn reject_dir_for_its_children(
    ...
    if !metadata.is_dir() { return Ok(true); }
    let mut read_dir = tokio::fs::read_dir(source.as_ref()).await.map_err(|_| Error::RuleParametersRMPDecode(rmp_serde::decode::Error::TypeMismatch(Marker::FixPos(0))))?;
    while let Ok(Some(entry)) = read_dir.next_entry().await {
        let name = match entry.file_name().into_string() { Ok(s) => s, Err(_) => continue };
        if entry.metadata().await.map(|m| m.is_dir()).unwrap_or(false) && children.contains(&name) { return Ok(false); }
    }
    Ok(true)
}

fn accept_by_glob(source: impl AsRef<Path>, accept_glob_set: &GlobSet) -> bool { accept_glob_set.is_match(source.as_ref()) }
fn reject_by_glob(source: impl AsRef<Path>, reject_glob_set: &GlobSet) -> bool { !accept_by_glob(source, reject_glob_set) }
...
impl From<SystemIndexerRule> for IndexerRule {
    fn from(rule: SystemIndexerRule) -> Self {
        Self { id: None, name: rule.name.to_string(), default: rule.default, rules: rule.rules, date_created: Utc::now(), date_modified: Utc::now() }
    }
}

impl From<&SystemIndexerRule> for IndexerRule {
    fn from(rule: &SystemIndexerRule) -> Self {
        Self { id: None, name: rule.name.to_string(), default: rule.default, rules: rule.rules.clone(), date_created: Utc::now(), date_modified: Utc::now() }
    }
}

pub static NO_SYSTEM_FILES: Lazy<SystemIndexerRule> = Lazy::new(|| SystemIndexerRule {
...
pub struct GitIgnoreRules { rules: RulePerKind }
...
impl GitIgnoreRules {
    pub async fn get_rules_if_in_git_repo(location_root: &Path, current: &Path) -> Option<Self> {
        ...
        for ancestor in current.ancestors().take_while(|p| p.starts_with(location_root)) {
            let gi = ancestor.join(".gitignore");
            if tokio::fs::try_exists(&gi).await.ok()? { ignores.push(gi); }
            if Self::is_git_repo(ancestor).await { git_repo = Some(ancestor.to_path_buf()); break; }
        }
    ...
    async fn parse_git_repo(git_repo: &Path, git_ignores: Vec<PathBuf>) -> Result<Self, ()> {
        ...
        for list in lists { search.patterns.push(list); }
        if let Ok(extra) = Self::parse_git_exclude(git_repo.join(".git")).await { search.patterns.extend(extra); }
        Ok(Self { rules: RulePerKind::IgnoredByGit(git_repo.to_path_buf(), search) })
    }

    async fn parse_git_ignore(path: PathBuf) -> Result<gix_ignore::glob::search::pattern::List<gix_ignore::search::Ignore>, ()> {
    ...
    async fn parse_git_exclude(dot_git: PathBuf) -> Result<Vec<gix_ignore::glob::search::pattern::List<gix_ignore::search::Ignore>>, ()> {
        tokio::task::spawn_blocking(move || {
            let mut buf = Vec::new();
    ...
    async fn is_git_repo(path: &Path) -> bool {
        let p = path.join(".git");
        tokio::task::spawn_blocking(move || p.is_dir()).await.unwrap_or(false)
    }
}

pub async fn build_default_ruler(toggles: RuleToggles, location_root: &Path, current: &Path) -> IndexerRuler {
    let mut base: Vec<IndexerRule> = Vec::new();
    if toggles.no_system_files { base.push((&*NO_SYSTEM_FILES).into()); }
    if toggles.no_hidden { base.push((&*NO_HIDDEN).into()); }
