...
use anyhow::Result;
use sea_orm::{prelude::*, DatabaseConnection, QuerySelect, Condition};
...
impl CopyDatabaseQuery {
    ...
    async fn get_path_estimates(&self, path: &Path) -> Result<Option<SinglePathEstimate>> {
        ...
        let locations_list = location::Entity::find()
            .all(&self.db)
            .await?;
        ...
        let relative_path = path.strip_prefix(&location_path)
        ...
        let (parent_path, name) = if let Some(pos) = relative_path.rfind('/') {
            (relative_path[..pos].to_string(), relative_path[pos + 1..].to_string())
        } else {
