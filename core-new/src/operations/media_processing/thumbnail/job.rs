...
use super::{
    ...
    state::{ThumbnailState, ThumbnailPhase, ThumbnailEntry, ThumbnailStats},
...
impl JobHandler for ThumbnailJob {
    ...
    async fn run(&mut self, ctx: JobContext<'_>) -> JobResult<Self::Output> {
        ...
        if self.state.as_ref().unwrap().phase == ThumbnailPhase::Processing {
            Self::run_processing_phase_static(
                &self.config,
                self.state.as_mut().unwrap(),
                &ctx,
            ).await?;
    ...
    async fn on_resume(&mut self, ctx: &JobContext<'_>) -> JobResult {
        if let Some(state) = &self.state {
            ...
            ctx.log(format!("Progress: {} generated, {} skipped, {} errors", 
    ...
    async fn on_cancel(&mut self, ctx: &JobContext<'_>) -> JobResult {
        ...
        if let Some(state) = &self.state {
            ctx.log(format!("Generated {} thumbnails before cancellation", 
                state.stats.generated_count));
        }
        Ok(())
    }
...
impl ThumbnailJob {
    ...
    async fn get_or_create_state(&mut self, ctx: &JobContext<'_>) -> JobResult<&mut ThumbnailState> {
    ...
    async fn run_discovery_phase_static(
        ...
        let mut mime_conditions = vec!["e.mime_type LIKE 'image/%'", "e.mime_type = 'application/pdf'"];
        ...
        for entry in entries {
            if !config.regenerate && Self::has_all_thumbnails_static(&entry.cas_id, config, ctx).await? {
        ...
        ctx.log(format!("Discovery complete: {} entries found, {} batches created", 
            state.stats.discovered_count, state.batches.len()));
        ...
        for (batch_idx, batch) in batches.iter().enumerate() {
            ...
            for (entry, result) in batch.iter().zip(results.iter()) {
                match result {
                    ...
                    Err(e) => {
                        let error_msg = format!("Failed to generate thumbnail for {}: {}", 
                            entry.relative_path, e);
                        state.add_error(error_msg.clone());
                        ctx.add_non_critical_error(crate::infrastructure::jobs::error::JobError::execution(error_msg));
                    }
                }
            }
    ...
    async fn generate_thumbnails_for_entry_static(
        ...
        for &size in &config.sizes {
            ...
            let thumbnail_info = generator.generate(
                &source_path,
                &thumbnail_path,
                size,
                config.quality,
            ...
            ctx.log(format!("Generated {}x{} thumbnail for {} ({}KB)", 
