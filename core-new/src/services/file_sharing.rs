...
use crate::{
    ...
    shared::types::SdPath,
    infrastructure::networking::protocols::file_transfer::FileMetadata,
};
...
impl FileSharingService {
    ...
    pub async fn share_with_device(
        ...
        let _networking = self.context.get_networking().await
            .ok_or(SharingError::NetworkingUnavailable)?;
            
        // Get the current library to access its job manager
        let library = self.context.library_manager.get_primary_library().await
            .ok_or(SharingError::JobError("No active library for job dispatch".to_string()))?;
