...
impl MacOSHandler {
    ...
    fn convert_notify_event(&self, notify_event: notify::Event) -> WatcherEvent {
        let kind = match notify_event.kind {
            ...
            EventKind::Modify(ModifyKind::Metadata(MetadataKind::WriteTime | MetadataKind::Extended)) => {
                WatcherEventKind::Modify
            }
            EventKind::Modify(ModifyKind::Name(RenameMode::Any)) => WatcherEventKind::Other("rename".to_string()),
    ...
    async fn handle_single_rename_event(
        ...
        match tokio::fs::metadata(&path).await {
            Ok(metadata) => {
                ...
                if let Some(inode) = self.get_inode_from_path(&path).await {
                    ...
                    if let Some((_, old_path)) = old_paths.remove(&inode) {
                        ...
                        trace!("Detected rename: {} -> {}", old_path.display(), path.display());
            ...
            Err(e) => {
                error!("Error accessing path during rename: {}: {}", path.display(), e);
            }
        }
    ...
    async fn process_event(
        ...
        match &event.kind {
            WatcherEventKind::Create => {
                ...
                if tokio::fs::metadata(&path).await.map_or(false, |m| m.is_dir()) {
            ...
            WatcherEventKind::Other(event_type) if event_type == "rename" => {
                ...
                let rename_events = self.handle_single_rename_event(path, watched_locations).await?;
...
impl MacOSHandler {
    ...
    pub async fn tick_with_locations(
        ...
        if last_check.elapsed() > HUNDRED_MILLIS {
            ...
            let create_events = self.handle_rename_create_eviction(watched_locations).await?;
            all_events.extend(create_events);

            // Handle rename remove evictions  
            let remove_events = self.handle_rename_remove_eviction(watched_locations).await?;
