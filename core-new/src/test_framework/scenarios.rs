...
pub async fn run_pairing_initiator(
	...
	let (pairing_code, expires_in) =
		timeout(Duration::from_secs(15), core.start_pairing_as_initiator()).await??;
	...
	println!(
		...
		device_name, short_code, expires_in
	);
	...
	println!("🔍 ALICE_DEBUG: Writing pairing code to file: {}", pairing_code);
	println!(
		"📝 {}: Pairing code written to /tmp/spacedrive-pairing-test/pairing_code.txt",
		device_name
	);
	...
	loop {
		...
		if !connected_devices.is_empty() {
			...
			println!(
				"PAIRING_SUCCESS: {} connected to Bob successfully",
				device_name
			);
			break;
		}
	...
	// FINAL FIX FOR TEST HARNESS: Keep Alice's process alive for a few seconds 
	// to ensure the test runner can see the success message before this process exits
	println!("⏳ {}: Waiting briefly to ensure test runner captures success...", device_name);
...
pub async fn run_pairing_joiner(
	data_dir: &Path,
	device_name: &str,
) -> Result<(), Box<dyn std::error::Error>> {
	println!("🟦 {}: Starting Core pairing test", device_name);
	...
	let pairing_code = loop {
		if let Ok(code) = std::fs::read_to_string("/tmp/spacedrive-pairing-test/pairing_code.txt") {
			...
			println!("🔍 BOB_DEBUG: Read pairing code from file: {}", trimmed_code);
			break trimmed_code;
		}
	...
	timeout(
		Duration::from_secs(45), // Increased timeout to allow robust polling mechanism to complete
	...
	loop {
		...
		if !connected_devices.is_empty() {
			...
			println!(
				"PAIRING_SUCCESS: {} connected to Alice successfully",
				device_name
			);
			break;
		}
...
pub async fn run_file_copy_sender(
	data_dir: &Path,
	device_name: &str,
) -> Result<(), Box<dyn std::error::Error>> {
	...
	println!(
		"🟦 {}: Starting cross-device file copy test (sender)",
		device_name
	);
	println!("📁 {}: Data dir: {:?}", device_name, data_dir);
	...
	println!(
		"🔑 {}: Starting pairing as initiator for file copy test...",
		device_name
	);
	let (pairing_code, expires_in) =
		timeout(Duration::from_secs(15), core.start_pairing_as_initiator()).await??;
	...
	std::fs::write(
		"/tmp/spacedrive-file-copy-test/pairing_code.txt",
		&pairing_code,
	)?;
	println!(
		"📝 {}: Pairing code written to /tmp/spacedrive-file-copy-test/pairing_code.txt",
		device_name
	);
	...
	loop {
		...
		if !connected_devices.is_empty() {
			...
			println!(
				"🎉 {}: Receiver connected! Device ID: {}",
				device_name, connected_devices[0]
			);
			println!(
				"🔍 ALICE_DEBUG: About to use device ID {} for file copy",
				connected_devices[0]
			);
			break;
		}
	...
	let test_files = vec![
		...
		(
			"metadata_test.json",
			r#"{"test": "file", "size": "medium", "purpose": "cross-device-copy"}"#,
		),
	];
	...
	match transfer_results {
		Ok(transfer_ids) => {
			...
			for transfer_id in &transfer_ids {
				println!(
					"🔍 ALICE_DEBUG: Checking status for transfer_id: {:?}",
					transfer_id
				);
						...
						Ok(status) => {
							match status.state {
								...
								crate::infrastructure::api::TransferState::Failed => {
									println!(
										"❌ {}: Transfer {:?} failed: {:?}",
										device_name, transfer_id, status.error
									);
									break;
								}
								_ => {
				...
				if !completed {
					println!(
						"⚠️ {}: Transfer {:?} did not complete in time",
						device_name, transfer_id
					);
				}
			}
...
pub async fn run_file_copy_receiver(
	data_dir: &Path,
	device_name: &str,
) -> Result<(), Box<dyn std::error::Error>> {
	println!(
		"🟦 {}: Starting cross-device file copy test (receiver)",
		device_name
	);
	println!("📁 {}: Data dir: {:?}", device_name, data_dir);
	...
	println!(
		"🔍 {}: Looking for pairing code from sender...",
		device_name
	);
	let pairing_code = loop {
	...
	println!(
		"📁 {}: Created directory for received files: {:?}",
		device_name, received_dir
	);

	// Wait for expected files to arrive
	let expected_files = loop {
		if let Ok(content) =
			std::fs::read_to_string("/tmp/spacedrive-file-copy-test/expected_files.txt")
		{
			break content
				.lines()
				.map(|line| {
	...
	println!(
		"📋 {}: Expecting {} files to be received",
		device_name,
		expected_files.len()
	);
	...
	if received_files.len() == expected_files.len() {
		println!(
			"✅ {}: All expected files received successfully!",
			device_name
		);
		...
		if verification_success {
			println!(
				"FILE_COPY_SUCCESS: {} verified all received files",
				device_name
			);
		} else {
