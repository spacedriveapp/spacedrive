...
use sea_orm::{EntityTrait, PaginatorTrait, ActiveModelTrait, QueryFilter, ColumnTrait, QuerySelect};
...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let library = if core.libraries.list().await.is_empty() {
        ...
        let lib = core.libraries.create_library("Desktop Demo Library", None).await?;
        ...
        while let Ok(event) = event_subscriber.recv().await {
            match event {
                ...
                Event::IndexingFailed { location_id, error } => {
                    println!("   ❌ Indexing failed for location: {} - {}", location_id, error);
                    break;
                }
                ...
                Event::JobProgress { job_id, progress, message } => {
                    ...
                    if let Some(msg) = message {
                        println!("   📊 Job {}: {} ({}%)", job_id, msg, (progress * 100.0) as u8);
                    } else {
        ...
        while let Ok(Some(entry)) = entries.next_entry().await {
            if let Ok(metadata) = entry.metadata().await {
                if metadata.is_file() {
                    ...
                    dir_count += 1;
                    if dir_count <= 3 { // Show first 3 dirs
                        println!("      📁 {}/", entry.file_name().to_string_lossy());
                    }
                }
            }
    ...
    loop {
        ...
        println!("   📊 Job Status: {} running, {} completed, {} total", 
                 running_jobs.len(), completed_jobs.len(), job_status.len());
        ...
        if checkpoint_estimate > last_checkpoint_size {
            println!("   📈 Progress detected: {} bytes checkpoint data", checkpoint_estimate);
        ...
        if running_jobs.is_empty() && !completed_jobs.is_empty() {
            ...
            println!("   ⚠️  Job appears stalled (no progress for {} seconds)", stall_timeout.as_secs());
    ...
    let path_prefix_count = entities::path_prefix::Entity::find()
        .count(db.conn())
        .await?;
    ...
    println!("      🔄 Entries with inode tracking: {} ({:.1}%)", 
             entries_with_inodes, 
             (entries_with_inodes as f64 / entry_count.max(1) as f64) * 100.0);
    println!("      🔗 Content identities created: {}", content_identity_count);
    println!("      📂 Path prefixes optimized: {}", path_prefix_count);
    ...
    for entry in &sample_entries {
        ...
        if entry.name == ".DS_Store" || entry.name == "Thumbs.db" {
            println!("      ❌ Found system file that should be filtered: {}", entry.name);
            filtered_correctly = false;
        }
        if entry.name == "node_modules" || entry.name == ".git" || entry.name == "__pycache__" {
            println!("      ❌ Found dev directory that should be filtered: {}", entry.name);
            filtered_correctly = false;
        }
    }
    ...
    println!("      🔄 Incremental Ready - {} entries have inode tracking", entries_with_inodes);
    println!("      📊 Batch Processing - Efficient memory usage");
    println!("      🎯 Multi-phase - Discovery → Processing → Content");
    println!("      💾 Path Optimization - {} prefixes reduce storage", path_prefix_count);
    println!("      🔍 Content Deduplication - {} unique content IDs", content_identity_count);
    ...
    let final_running = library.jobs().list_jobs(Some(
        sd_core_new::infrastructure::jobs::types::JobStatus::Running
    )).await?;
    let final_completed = library.jobs().list_jobs(Some(
        sd_core_new::infrastructure::jobs::types::JobStatus::Completed
    )).await?;
