...
impl PairingUserInterface for LibP2PUI {
	...
	async fn prompt_pairing_code(&self) -> Result<[String; 12]> {
		...
		Err(NetworkError::AuthenticationFailed(
			"Invalid pairing code format".to_string(),
		))
	}
	...
	async fn confirm_pairing(&self, remote_device: &DeviceInfo) -> Result<bool> {
		...
		println!("\n🔐 Pairing Request");
		println!(
			"Device '{}' wants to pair with this device.",
			remote_device.device_name
		);
	...
	async fn show_pairing_progress(&self, state: PairingState) {
		match state {
			...
			PairingState::Broadcasting => {
				println!("📡 Providing on Kademlia DHT (global discovery)...")
			}
			...
			PairingState::Authenticating => {
				println!("🔐 Authenticating via request-response protocol...")
			}
			PairingState::ExchangingKeys => {
				println!("🔄 Exchanging device information over libp2p...")
			}
...
async fn create_test_device(name: &str) -> Result<(DeviceInfo, PrivateKey)> {
	...
	println!(
		"   🔐 Network Fingerprint: {}",
		device_info.network_fingerprint
	);
...
async fn run_pairing_initiator() -> Result<()> {
	...
	match pairing_protocol.start_as_initiator(&*ui).await {
		Ok((remote_device, session_keys)) => {
			...
			println!(
				"   🔑 Receive key: {}",
				hex::encode(session_keys.receive_key)
			);
