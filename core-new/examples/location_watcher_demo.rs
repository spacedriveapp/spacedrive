...
use sd_core_new::{Core, infrastructure::events::{Event, EventFilter}};
...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let library_id = library.id().await;
    ...
    core.add_watched_location(location_id, library_id, watch_dir.clone(), true).await?;
        ...
        while let Ok(event) = event_subscriber.recv().await {
            match event {
                Event::EntryCreated { library_id, entry_id } => {
                    info!("📁 File created - Library: {}, Entry: {}", library_id, entry_id);
                }
                Event::EntryModified { library_id, entry_id } => {
                    info!("✏️  File modified - Library: {}, Entry: {}", library_id, entry_id);
                }
                Event::EntryDeleted { library_id, entry_id } => {
                    info!("🗑️  File deleted - Library: {}, Entry: {}", library_id, entry_id);
                }
                Event::EntryMoved { library_id, entry_id, old_path, new_path } => {
                    info!("📦 File moved - Library: {}, Entry: {}, {} -> {}", 
    ...
    info!("Renamed file: {} -> {}", nested_file.display(), renamed_file.display());
    sleep(Duration::from_millis(200)).await;
