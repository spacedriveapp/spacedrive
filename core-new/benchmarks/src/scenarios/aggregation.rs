...
#[derive(Default)]
pub struct AggregationScenario {
...
impl Scenario for AggregationScenario {
	fn name(&self) -> &'static str {
		"aggregation"
	}

	fn describe(&self) -> &'static str {
		"Measure aggregation-inclusive processing throughput (excludes discovery and content)"
	}
	...
	async fn prepare(&mut self, boot: &CoreBoot, recipe: &Recipe) -> Result<()> {
		...
		// Add locations in Shallow mode (Discovery -> Processing -> Aggregation)
	...
	async fn run(&mut self, _boot: &CoreBoot, recipe: &Recipe) -> Result<Vec<BenchmarkRun>> {
		...
		println!(
			"Waiting for {} aggregation jobs to finish...",
			self.job_ids.len()
		);
		loop {
		...
		// Parse logs; use processing duration as scenario duration (aggregation is included in processing phase timing)
		...
		for jid in &self.job_ids {
			...
			if let Ok(txt) = std::fs::read_to_string(&log_path) {
				for cap in re.captures_iter(&txt) {
					...
					if let Some(d) = cap.get(1) {
						duration_total = d.as_str().parse::<f64>().ok();
					}
					...
					// Phase timings: discovery (8), processing (9), content (10)
					if let Some(dd) = cap.get(8) {
						discovery = dd.as_str().parse::<f64>().ok();
					}
					if let Some(pp) = cap.get(9) {
						processing = pp.as_str().parse::<f64>().ok();
					}
					if let Some(cd) = cap.get(10) {
						content = cd.as_str().parse::<f64>().ok();
					}
				}
			}
			let files_v = files.unwrap_or_default();
			let proc_secs = processing.or(duration_total).unwrap_or_default();
			let files_per_s = if proc_secs > 0.0 {
				files_v as f64 / proc_secs
			} else {
				0.0
			};
			...
			let meta = RunMeta {
				...
				hardware_label: crate::metrics::derive_hardware_label_from_paths(&location_paths)
					.or_else(|| self.hardware_hint.as_ref().and_then(|h| hardware_hint_to_label(h)))
