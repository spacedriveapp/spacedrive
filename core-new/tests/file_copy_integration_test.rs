...
use sd_core_new::{
    ...
    shared::types::{SdPath, SdPathBatch, set_current_device_id},
};
...
/// Mock JobContext for testing (simplified)
#[derive(Debug)]
...
impl MockJobContext {
    ...
    async fn check_interrupt(&self) -> Result<(), Box<dyn std::error::Error>> {
        if self.interrupted {
    ...
    async fn checkpoint(&self) -> Result<(), Box<dyn std::error::Error>> {
        Ok(())
    }
    
    fn progress(&self, _progress: ()) {
        // Mock progress
    }
    
    fn log(&self, message: String) {
        println!("Job log: {}", message);
    }
    
    fn add_non_critical_error(&self, error: String) {
        println!("Non-critical error: {}", error);
    }
}
...
async fn test_file_copy_directory_structure() {
    ...
    for (filename, expected_content) in &test_files {
        ...
        assert_eq!(*expected_content, actual_content, "Content mismatch for {}", filename);
    }
...
async fn test_copy_job_with_options() {
    ...
    create_test_file(&source_path, "test content").await.unwrap();
...
async fn test_cross_device_detection() {
    ...
    assert_ne!(copy_job.sources.paths[0].device_id, copy_job.destination.device_id);
...
async fn test_file_operations_integration() {
    ...
    for (i, (_, expected_content)) in files.iter().enumerate() {
        if let Some(local_path) = copy_job.sources.paths[i].as_local_path() {
            ...
            assert_eq!(*expected_content, actual_content, "Content mismatch for file {}", i);
        }
    }
